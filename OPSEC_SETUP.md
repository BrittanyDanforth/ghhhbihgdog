...
below_threshold / below_floor / nothing / wallet_is_mixing /
wallet_unverified` — kinds, never a figure or an address. The job log (0600, wiped with the artifacts) carries GhostSpiral's
...
- **A mix's accounts are on disk before money reaches them.** wallet-rpc
  keeps a new account in memory until it is asked to store, or stops in an
...
**Kerckhoffs on the chain.** The forward is one transaction with no change
output, so what it pays the miner is a public number — and it used to be
EXACTLY the vsize bound times an integer rate, while the limit it wrote
into the memo was EXACTLY 99 times an integer (six decimals of Monero,
...
`--btc-electrum` is not optional on an intake pair, and the pager enforces
it: a card from an intake pairing (it carries the intake floor) refuses to
start without the flag, and a card from before that field refuses the first
intake deposit before its address is shown (`intake_unwatched` on the
pager's chain). Each server must be a `.onion` or pinned
(`host:port,<sha256>`), as on the vault: this end's looks decide what the
client is told and when a forward starts, and an unpinned clearnet server
is whatever the Tor exit says it is. The flag is also how the
