...
DR = "http://127.0.0.1:30161"; D = DR + "/json_rpc"; WR = "http://127.0.0.1:30163/json_rpc"
...
try:
    Lp(["monerod", "--testnet", "--offline", "--data-dir", os.path.join(BASE, "n"),
        "--rpc-bind-ip", "127.0.0.1", "--rpc-bind-port", "30161", "--p2p-bind-port", "30160",
    ...
    Lp(["monero-wallet-rpc", "--testnet", "--daemon-address", "127.0.0.1:30161", "--trusted-daemon",
        "--wallet-dir", os.path.join(BASE, "w"), "--rpc-bind-port", "30163", "--rpc-bind-ip", "127.0.0.1",
