...
MUTATIONS = [
 ...
 ("a bare forwarded to a start from seen keeps the first forward's clock", 'gs_telegram_pager',
  ...
  '                if False:\n                    e["sent_at"] = time.time()\n                e["state"] = "forwarded"\n                e["word"] = phase\n                e["held_conf"] = _start_conf',
  ['test_telegram_pager']),
 ("an entry whose hold owes an ask is forgotten on the forward's clock", 'gs_telegram_pager',
  '                           >= proto.DEPOSIT_PLACE_TTL_S\n                           and not self._btc_hold_runs(v, _now))]:',
  '                           >= proto.DEPOSIT_PLACE_TTL_S)]:',
  ['test_telegram_pager']),
 ('no slack after a timed hold: forgotten on the tick its ask was due', 'gs_telegram_pager',
  '        return now < _u + float(self.HOLD_SLACK_WINDOWS) * float(\n            self.btc_recheck_s)',
  '        return now < _u',
  ['test_telegram_pager']),
