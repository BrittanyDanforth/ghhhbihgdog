...
MUTATIONS = [
 ...
 ('a deposit that can never be sent on is answered delayed, not short',
  ...
  ['test_btc_forwarder']),
 ('short or delayed asks only the floor rate: a forward that passes only '
  'where the small outputs are dust reads short', 'btc_forwarder',
 ...
 ('short or delayed ignores the fee guards: any selection reads as a '
  ...
  '    return True',
  ['test_btc_forwarder']),
 ('short or delayed gives up the band the wrong way: money under the '
  'one-input floor reads as a forward', 'btc_forwarder',
  '                op_return_limit, r, min_send_sat):\n            return False',
  '                op_return_limit, r, min_send_sat):\n            return True',
  ['test_btc_forwarder']),
 ...
 ('a dust flood makes a forward no node relays', 'btc_forwarder',
  '    if not chosen or standard_fits(len(chosen), op_return_limit):',
  '    if True:',
  ['test_btc_forwarder']),
 ...
 ("the cap drops a replacement's committed inputs", 'btc_forwarder',
  ...
  '    keep = set()',
  ['test_btc_forwarder']),
 ('the band stops one rate short of the ceiling', 'btc_forwarder',
  '                       if floor < r <= ceiling}',
  '                       if floor < r < ceiling}',
  ['test_btc_forwarder']),
