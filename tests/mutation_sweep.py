...
MUTATIONS = [
 ...
 ('the signed hex IS kept after the network showed the transaction', 'btc_forwarder',
  ...
  '    else:\n        hex_reason = "unseen"',
  ['test_btc_forwarder']),
 ('a forward that may have moved money is not marked sent', 'gs_wake_agent',
  '        handles[handle]["forward_sent"] = bool(\n            _forward_outcome(artifact_dir, handle)[0])',
  '        handles[handle]["forward_sent"] = False',
  ['test_wake_agent']),
 ...
 ('a deposit address is issued without the constant-time backend', 'gs_wake_agent',
  ...
  '    try:\n        pass\n    except _btx.BtcTxError:\n        integrity_log("wake", "btc_backend_missing")',
  ['test_wake_agent']),
 ('the forwarder looks and quotes before asking for the backend', 'btc_forwarder',
