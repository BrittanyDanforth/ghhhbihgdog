...
MUTATIONS = [
 ...
 ("the veil delay is a fixed value, not jittered", "GhostSpiral",
  '            "delay": hop_delay(delay_window),\n        }\n        # HOW MANY OUTPUTS THIS SWEEP SPENDS',
 ...
 ('the pager treats a dropped reply as a delivered one again', 'gs_telegram_pager',
  '            _LAST_MID.failed = True\n            return False\n',
  '            _LAST_MID.failed = True\n            return True\n',
  ['test_sealed_slip']),
 ...
 ("the unreadable-quote note goes back to describing a replaced target",
  "receive_watch",
  '            if args.expect_xmr is None:\n                print(f"  [!] They contribute NOTHING',
  '            if True:\n                print(f"  [!] They contribute NOTHING',
  ["test_listed_bugs"]),
 ...
 ('the withdrawal completion notice goes back to one unchecked send', 'gs_telegram_pager',
  "                    if not self.send(chat_id, _msg):\n                        # The leg's report is lost: _worker says so on the\n",
  "                    if False:\n                        # The leg's report is lost: _worker says so on the\n",
  ['test_plain_slip']),
 ...
 ("the capacity refusal carries no word", "gs_wake_agent",
  ...
  '_REFUSAL_PHASE = {"at_capacity": "", "intake_unwatched": "unwatched"}',
  ["test_multi_client"]),
 ...
 ('the address is sent even when the note did not get through', 'gs_telegram_pager',
  '                if not _ok:\n                    _LAST_MID.failed = True\n                    self._mark_undelivered(chat_id, h, job)\n',
  '                if False:\n                    _LAST_MID.failed = True\n                    self._mark_undelivered(chat_id, h, job)\n',
  ['test_plain_slip']),
 ...
 ('the watch-only Pi accepts an xPRV where an xPUB belongs', 'gs_btc_watch.py',
  ...
  '    if False:',
  ['test_btc_watch']),
 ('an xpub for another network or script type derives a quiet wrong address',
  'gs_btc_watch.py',
 ...
 ('look() stops isolating each address on its own Tor circuit', 'gs_btc_watch.py',
  ...
  ['test_btc_watch']),
 ('a proxy URL carrying its own credential is used as-is (one circuit for all)',
  'gs_btc_watch.py',
  '    if "@" in rest:',
  '    if False:',
  ['test_btc_watch']),
 ('an output below the confirmation threshold is counted as settled',
  'gs_btc_watch.py',
  '        if depth >= min_conf:',
  '        if depth >= 0:',
  ['test_btc_watch']),
 ('a proxy that downgrades to no-auth is accepted, silently un-isolated',
  'gs_btc_watch.py',
  '        if method != offered:',
  '        if False:',
  ['test_btc_watch']),
 ('the deposit destination is sent as a raw-IP ATYP, not a domain for the proxy',
  'gs_btc_watch.py',
  '        sock.sendall(bytes([_SOCKS_VER, _CMD_CONNECT, 0x00, _ATYP_DOMAIN,',
  '        sock.sendall(bytes([_SOCKS_VER, _CMD_CONNECT, 0x00, _ATYP_IPV4,',
  ['test_btc_watch']),
 ('the whole-exchange deadline is never armed: a trickling server holds the watcher',
  'gs_btc_watch.py',
 ...
 ("a server's error TEXT is copied into the error the caller logs",
  ...
  '            and -32768 <= code <= 32767:\n        return code\n    return str(err)',
  ['test_btc_watch']),
 ("a server's unbounded integer 'code' (a scripthash in base 10) is passed through",
  'gs_btc_watch.py',
  '            and -32768 <= code <= 32767:',
  '            and True:',
  ['test_btc_watch']),
 ('the pin from a (host, port, pin) entry never reaches the real transport',
  'gs_btc_watch.py',
  '                                          timeout=timeout, pin=pin)',
  '                                          timeout=timeout, pin=None)',
  ['test_btc_watch']),
 ("look()'s timeout never reaches the real transport (the default is used)",
  'gs_btc_watch.py',
  '                                          timeout=timeout, pin=pin)',
  '                                          timeout=DEFAULT_TIMEOUT, pin=pin)',
  ['test_btc_watch']),
 ('a pin mismatch (a detected interception) is absorbed by failover',
  'gs_btc_watch.py',
  '        except PinMismatch:',
  '        except ():',
  ['test_btc_watch']),
 ('a mempool output (Electrum height 0) is treated as mined and can settle',
  'gs_btc_watch.py',
  '        if height > 0:',
  '        if height >= 0:',
  ['test_btc_watch']),
 ('min_conf=0 is accepted: an unconfirmed deposit counts as settled money',
  'gs_btc_watch.py',
  '    if not _is_uint(min_conf) or min_conf < 1:',
  '    if not _is_uint(min_conf):',
  ['test_btc_watch']),
 ('every mined output reads one block deeper than it is (1-deep passes min_conf=2)',
  'gs_btc_watch.py',
  '            depth = max(1, tip - height + 1)',
  '            depth = max(1, tip - height + 2)',
  ['test_btc_watch']),

 # ---- stage 2: build, sign, forward (gs_btc_tx, btc_forwarder, the job) ----
 ('signing proceeds on the variable-time pure-Python curve', 'gs_btc_tx.py',
  '    if not (_curve.NATIVE and _curve.BACKEND in ("libsecp256k1",\n                                                  "micropython")):',
  '    if False:',
  ['test_btc_tx', 'test_btc_forwarder']),
 ('a 76-byte OP_RETURN push is laid out as a direct push (malformed script)',
  'gs_btc_tx.py',
  '    if n <= 75:',
  '    if n <= 76:',
  ['test_btc_tx']),
 ('the BIP143 digest is computed over the scriptPubKey, not the script code',
  'gs_btc_tx.py',
  '    code = script.p2pkh(pub)                   # BIP143 script code, not spk',
  '    code = script.p2wpkh(pub)                  # BIP143 script code, not spk',
  ['test_btc_tx', 'test_btc_forwarder']),
 ("an inbound address of another network becomes this network's output",
  'gs_btc_tx.py',
  '    witver, prog = bech32.decode(net["bech32"], addr.lower())',
  '    witver, prog = bech32.decode(addr.lower().split("1")[0], addr.lower())',
  ['test_btc_tx']),
 ('a transaction whose outputs exceed its inputs is built', 'gs_btc_tx.py',
  '    if total_out > total_in:\n        raise BtcTxError("outputs exceed inputs")',
  '    if False:\n        raise BtcTxError("outputs exceed inputs")',
  ['test_btc_tx']),
 ('a memo over the OP_RETURN policy is signed anyway', 'btc_forwarder',
  '    if memo_will_overflow(memo_text, op_return_limit):',
  '    if False:',
  ['test_btc_forwarder']),
 ('the final memo is not re-checked for the destination after its limit '
  'was written', 'btc_forwarder',
  '    if not _memo_fields_bind(memo_text, dest_xmr):\n        raise Refused("memo_unbound", "the memo no longer names this "',
  '    if False:\n        raise Refused("memo_unbound", "the memo no longer names this "',
  ['test_btc_forwarder']),
 ("a memo naming someone else's XMR destination is signed", 'btc_forwarder',
  '    if not memo_binds_destination(text, dest_xmr):',
  '    if False:',
  ['test_btc_forwarder']),
 ('a seed that does not derive the watched xpub signs anyway', 'btc_forwarder',
  '    if not btx.account_matches_xpub(account, account_xpub):',
  '    if False:',
  ['test_btc_forwarder']),
 ('a mined-but-shallow output (under min_conf) becomes an input of the forward',
  'btc_forwarder',
 ...
 ("an inbound address of another network is paid", 'btc_forwarder',
  ...
  '    if False:',
  ['test_btc_forwarder']),
 ('a fee eating more than a fifth of the deposit is paid', 'btc_forwarder',
  '    if fee_sat > settled_sat * MAX_FEE_FRACTION:',
  '    if False:',
  ['test_btc_forwarder']),
 ('the forward is gated on allow_withdraw instead of its own switch', 'gs_wake_agent',
 ...
 ('the xpub goes back onto the argv', 'gs_wake_agent',
  ...
  '            env_extra["GS_BTC_XPUB"] = ""; argv_leak = str(key["btc_account_xpub"])',
  ['test_wake_agent']),
 ('a client token may forward another client\'s deposit (the owner wall is gone)',
  'gs_wake_agent',
  '        if _owner and _owner != proto.HOST_OWNER and rec.get("owner") \\\n                and rec.get("owner") != _owner:',
  '        if False:',
  ['test_wake_agent']),
 ('a rehearsed handle is rehearsed again', 'gs_wake_agent',
  '        if rec.get("forwarded") and not _btc_broadcast_on(key):',
  '        if False:',
  ['test_wake_agent']),
 ('a handle whose forward was SENT is forwarded again', 'gs_wake_agent',
  '        if _sent_before:',
  '        if False:',
  ['test_wake_agent']),
 ('the once-sent rule trusts the ledger alone, not the plan on disk',
  'gs_wake_agent',
 ...
 ('a paired BTC setting present and wrong is coerced, not refused', 'gs_wake_agent',
  ...
  '    if False:',
  ['test_wake_agent']),
 ('the plan file says broadcast: true for a dry run', 'btc_forwarder',
  '        "broadcast": moved,',
  '        "broadcast": True,',
  ['test_btc_forwarder']),
 ('the seed is read but left in the environment', 'btc_forwarder',
  '    words = os.environ.pop(SEED_ENV)',
  '    words = os.environ[SEED_ENV]',
  ['test_btc_forwarder']),
 ('the fee fraction is raised so a fee of a quarter of the deposit is paid',
  'gs_btc_tx.py',
  'FORWARD_MAX_FEE_FRACTION = Decimal("0.20")',
  'FORWARD_MAX_FEE_FRACTION = Decimal("0.99")',
  ['test_btc_forwarder', 'test_btc_tx']),
 ('the fee bound budgets a P2WPKH inbound, so a P2WSH/P2TR vault is paid under the target',
  'gs_btc_tx.py',
  'INBOUND_SPK_MAX = 34',
  'INBOUND_SPK_MAX = 22',
  ['test_btc_forwarder']),
 ('a hex-encoded memo is embedded as ASCII hex', 'btc_forwarder',
  '    if not _memo_fields_bind(text, dest_xmr):',
  '    if False:',
  ['test_btc_forwarder']),
 ('a zero or low output limit (a swap at any price) is laid out as quoted',
  'btc_forwarder',
  '    limit_set = quoted < floor',
  '    limit_set = False',
  ['test_btc_forwarder']),
 ('the floor is computed but the quoted limit is what goes on chain',
  'btc_forwarder',
  '    limit = floor if limit_set else quoted',
  '    limit = quoted',
  ['test_btc_forwarder']),
 ('a limit above the expected output (a certain refund) is signed',
  'btc_forwarder',
  '    if quoted > ceiling:',
  '    if False:',
  ['test_btc_forwarder']),
 ...
 ('the rewritten limit is computed but never written into the memo',
  ...
  '    if False:\n        sub[0] = str(limit)',
  ['test_btc_forwarder']),
 ('an affiliate skim over the cap is signed', 'btc_forwarder',
  '    if bps > max_affiliate_bps:',
  '    if False:',
  ['test_btc_forwarder']),
 ('the signed bytes are persisted in the plan file by default', 'btc_forwarder',
  '        "tx_hex": tx.serialize().hex() if hex_reason else None,',
  '        "tx_hex": tx.serialize().hex(),',
  ['test_btc_forwarder']),
 ('a halted or paused chain is paid', 'btc_forwarder',
  '    if any(entry.get(k) for k in ("halted", "chain_trading_paused",\n                                   "global_trading_paused",\n                                   "chain_lp_actions_paused")):',
  '    if False:',
  ['test_btc_forwarder']),
 ("an inbound THORNode does not publish is paid anyway", 'btc_forwarder',
  '    if str(entry.get("address") or "").lower() != str(inbound).lower():',
  '    if False:',
  ['test_btc_forwarder']),
 ('a forward under the minimum is signed (ThorChain would treat it as dust)',
  'btc_forwarder',
  '    if send < min_send_sat:',
  '    if False:',
  ['test_btc_forwarder']),
 ('a forward under the LIVE dust threshold is signed', 'btc_forwarder',
  '        if dust is not None and send_sat < dust:',
  '        if False:',
  ['test_btc_forwarder']),
 ('the forward is not a spending job (no deadman extension, no switch)', 'gs_wake_proto.py',
  'SPENDING_JOBS = ("withdraw", "forward_to_swap")',
  'SPENDING_JOBS = ("withdraw",)',
  ['test_wake_protocol']),
 # ---- stage 3: the broadcast (gs_btc_broadcast, --broadcast, the word) ----
 ('submit treats a FOREIGN txid as accepted', 'gs_btc_broadcast.py',
  '        if got != want:',
  '        if False:',
  ['test_btc_broadcast']),
 ('submit keeps routing around a PinMismatch (an interception is a dead server)',
  'gs_btc_broadcast.py',
  '            if ambiguous:\n                pinned = True\n                break\n            raise',
  '            continue',
  ['test_btc_broadcast']),
 ('submit ignores the sent flag: a hang-up after the bytes left reads as unreachable',
  'gs_btc_broadcast.py',
  '            if b.sent:\n                ambiguous = True',
  '            if False:\n                ambiguous = True',
  ['test_btc_broadcast']),
 ('ambiguous is reported as rejected or unreachable (money may have moved, said not to)',
  'gs_btc_broadcast.py',
  '    if ambiguous:\n        outcome = OUTCOME_AMBIGUOUS\n    elif codes:',
  '    if False:\n        outcome = OUTCOME_AMBIGUOUS\n    elif codes:',
  ['test_btc_broadcast']),
 ('seen counts ANY history entry as our transaction', 'gs_btc_broadcast.py',
  '        hit = next((e for e in entries if e["tx_hash"] == txid), None)',
  '        hit = entries[0] if entries else None',
  ['test_btc_broadcast']),
 ('the forwarder broadcasts under --dry-run', 'btc_forwarder',
  '    if args.broadcast:\n        age = _clock() - quoted_at',
  '    if True:\n        age = _clock() - quoted_at',
  ['test_btc_forwarder']),
 ('a stale quote is sent', 'btc_forwarder',
  '        if age > args.quote_max_age:',
  '        if False:',
  ['test_btc_forwarder']),
 ('a signed transaction under the rate floor is sent', 'btc_forwarder',
  '    if signed and real_rate < args.feerate_floor:',
  '    if False:',
  ['test_btc_forwarder']),
 ('a rejected-by-every-server forward is recorded as done', 'btc_forwarder',
  '        if outcome == bcast.OUTCOME_REJECTED:',
  '        if False:',
  ['test_btc_forwarder']),
 ('an ambiguous forward exits non-zero (rendered as "failed" about money in flight)',
  'btc_forwarder',
  '        if outcome == bcast.OUTCOME_UNREACHABLE:',
  '        if outcome in (bcast.OUTCOME_UNREACHABLE, bcast.OUTCOME_AMBIGUOUS):',
  ['test_btc_forwarder']),
 ('the signed hex is NOT kept when the network has not shown the transaction',
  'btc_forwarder',
  '    elif moved and not seen_r["seen"]:',
  '    elif False:',
  ['test_btc_forwarder']),
 ('the signed hex IS kept after the network showed the transaction', 'btc_forwarder',
  '    else:\n        hex_reason = None',
  '    else:\n        hex_reason = "unseen"',
  ['test_btc_forwarder']),
 ...
 ('a plan whose two broadcast fields disagree is read as moved', 'gs_wake_agent',
  ...
  '        moved = plan.get("broadcast") is True',
  ['test_wake_agent']),
 ('an ambiguous forward is reported as "sent"', 'gs_wake_agent',
 ...
 ('a non-boolean allow_btc_broadcast is coerced', 'gs_wake_agent',
  ...
  '    if False:',
  ['test_wake_agent']),
 ('pairing accepts the sending switch without the signing switch', 'gs_wake_keys',
  '    if args.allow_btc_broadcast and not args.allow_btc_forward:',
  '    if False:',
  ['test_wake_agent']),
 ('the two forward words leave the closed vocabulary', 'gs_wake_proto.py', '          "more_locked", "moved", "partial", "full", "sent", "unsure",\n          "delayed", "returned", "forwarded", "kept", "leftover",\n', '          "more_locked", "moved", "partial", "full",\n          "delayed", "returned", "forwarded", "kept", "leftover",\n', ['test_wake_agent']),
 ('the old plan is rotated aside before the fresh forward runs (the no_plan '
  ...
  ['test_btc_forwarder']),
 ('a rotation writes over a rotated plan that already has its number',
  'btc_forwarder',
  '    while target.exists():\n        n += 1\n        target = p.with_name(f"{stem}.{n}.json")\n    os.replace(p, target)',
  '    while False:\n        n += 1\n        target = p.with_name(f"{stem}.{n}.json")\n    os.replace(p, target)',
  ['test_btc_forwarder']),
 ('an adopted forward of ours is not recorded as a plan', 'btc_forwarder',
  '            _adopted.append((s, _memo))',
  '            pass',
  ['test_btc_forwarder']),
 ('the rebuilt plan is not counted in the run that adopts it', 'btc_forwarder',
  '        chain.append(_ap)\n        try:',
  '        try:',
  ['test_btc_forwarder']),
 ('the rebuilt plan carries no round of returned money', 'btc_forwarder',
  '        _ap["carried_refunds"] = len(_new & _rounds)',
  '        _ap["carried_refunds"] = 0',
  ['test_btc_forwarder']),
 ("a rebuilt replacement counts the replaced plan's inputs as a round again", 'btc_forwarder',
  '        _new = _outpoints(_ap["inputs"]) - _signed_ops',
  '        _new = _outpoints(_ap["inputs"])',
  ['test_btc_forwarder']),
 ('the rebuilt plan does not say how many inputs came back', 'btc_forwarder',
  '        _ap["carried_returned"] = len(_new)',
  '        _ap["carried_returned"] = None',
  ['test_btc_forwarder']),
 ('the rebuilt plan is not marked adopted', 'btc_forwarder',
  '        _ap["adopted"] = True',
  '        pass',
  ['test_btc_forwarder']),
 ('a rebuilt plan the disk refused is not counted for the run', 'btc_forwarder',
  '        chain.append(_ap)\n        try:\n            _where = _file_rotated(args.outfile, _ap)\n',
  '        try:\n            _where = _file_rotated(args.outfile, _ap)\n            chain.append(_ap)\n',
  ['test_btc_forwarder']),
 ('the rebuilt plan is written over the current plan', 'btc_forwarder',
  '    write_plan(target, plan)\n    return target',
  '    write_plan(p, plan)\n    return target',
  ['test_btc_forwarder']),
 ('the rebuilt plan is written over a rotated plan that has its number', 'btc_forwarder',
  '    while target.exists():\n        n += 1\n        target = p.with_name(f"{stem}.{n}.json")\n    write_plan(target, plan)',
  '    while False:\n        n += 1\n        target = p.with_name(f"{stem}.{n}.json")\n    write_plan(target, plan)',
  ['test_btc_forwarder']),
 ('a fee is recorded from inputs the history did not name', 'btc_forwarder',
  '               if _all and _named == _all else None)',
  '               if _all else None)',
  ['test_btc_forwarder']),
 ('a negative fee is recorded on a rebuilt plan', 'btc_forwarder',
 ...
 ('money that came back is forwarded beside a stuck forward, not in its replacement',
  ...
  '            integrity_log("forward", "returned_settled")',
  ['test_btc_forwarder']),
 ('the superseded path never bumps a stuck predecessor', 'btc_forwarder', '        _then = _after_record(stuck)\n        if _then is not None:\n            return _then\n        return "done", EXIT_OK', '        _then = _after_record(None)\n        if _then is not None:\n            return _then\n        return "done", EXIT_OK', ['test_btc_forwarder']),
 ...
 ('the recheck is never due', 'gs_telegram_pager',
  '        if e.get("word") == "unsure":\n            return True\n        return (now - float(e.get("sent_at") or now)\n                >= self._btc_recheck_after(e))',
  '        return False',
  ['test_telegram_pager']),
 ...
 ('the forgotten entry keeps routing to the XMR side', 'gs_telegram_pager',
  ...
  '                # NOTHING ON THE CHAIN FOR IT',
  '                # NOTHING ON THE CHAIN FOR IT',
  ['test_telegram_pager']),
 ('forwarded does not end the rechecks', 'gs_telegram_pager', '                e["state"] = "forwarded"\n                e["word"] = phase\n                e["sent_at"] = time.time()', '                e["state"] = "sent"\n                e["word"] = phase\n                e["sent_at"] = time.time()', ['test_telegram_pager']),
 ('a delayed replacement re-watches a sent deposit', 'gs_telegram_pager',
  ...
  '                    # A REPLACEMENT THE VAULT WOULD NOT PAY FOR TODAY',
  ['test_telegram_pager']),
 ('an unsure answer restarts the automatic recheck at the next tick (a wake a tick)', 'gs_telegram_pager', '                e["state"] = "sent"\n                e["word"] = phase\n                e["sent_at"] = time.time()\n                e["recheck_after"] = self._btc_recheck_draw()\n                e.pop("retry_after", None)\n                e.pop("fee_tries", None)\n                self._btc_unhold(e)\n                return', '                e["state"] = "sent"\n                e["word"] = phase\n                e["sent_at"] = time.time()\n                e["recheck_after"] = self._btc_recheck_draw()\n                e.pop("retry_after", None)\n                e.pop("fee_tries", None)\n                e.pop("rechecked_at", None)\n                self._btc_unhold(e)\n                return', ['test_telegram_pager']),
 ('the fee retry count is never cleared by a sent answer', 'gs_telegram_pager', '                e.pop("retry_after", None)\n                e.pop("fee_tries", None)\n                self._btc_unhold(e)\n                return\n            if out == "done" and phase == "leftover":', '                e.pop("retry_after", None)\n                self._btc_unhold(e)\n                return\n            if out == "done" and phase == "leftover":', ['test_telegram_pager']),
 ...
 ('a held fee retry is said as if it were a first forward', 'gs_telegram_pager',
  ...
  '                say = "held"',
  ['test_telegram_pager']),
 ('a refusal about an unpaid deposit marks it seen and ends the hold',
  'gs_telegram_pager',
  '            if e["state"] == "not_seen":\n'
 ...
 ('pairing takes a negative returns bound',
  ...
  ['test_wake_agent']),
 ("the wire does not know `kept` (the doorbell refuses the vault's report)", 'gs_wake_proto.py', '          "delayed", "returned", "forwarded", "kept", "leftover",\n', '          "delayed", "returned", "forwarded", "leftover",\n', ['test_wake_agent']),
 ('a kept forward leaves the entry sent (rechecked every window for ever)',
  ...
  '                # MONEY CAME BACK AGAIN AND THE VAULT KEEPS IT (third',
  ['test_telegram_pager']),
 ('the operator is not told a forward was stopped', 'gs_telegram_pager', '            if out == "failed" or _early in ("kept", "leftover"):', '            if out == "failed":', ['test_telegram_pager']),
 ...
 ('a kept deposit never asks the forward again (the operator\'s remedy unheard)',
  ...
  '                return False',
  ['test_telegram_pager']),
 ('a kept answer is not learned back after a restart', 'gs_telegram_pager', '                    and phase in ("sent", "unsure", "forwarded", "kept",\n                                  "leftover")', '                    and phase in ("sent", "unsure", "forwarded",\n                                  "leftover")', ['test_telegram_pager']),
 ('the early wait is dropped before the start is known to have happened',
  'gs_telegram_pager',
  '                    e["state"] = "forwarding"\n'
  '                    # ...AND THE TOTAL THIS START WAS MADE FOR, as the',
  '                    e["state"] = "forwarding"\n'
  '                    e.pop("retry_after", None)\n'
  '                    # ...AND THE TOTAL THIS START WAS MADE FOR, as the',
  ['test_telegram_pager']),
 ...
 ('the operator moving kept money by hand raises the seed-leak alarm',
  'btc_forwarder',
  '        if _hand:\n            # MONEY THE VAULT KEPT, MOVED BY HAND',
  '        if False:\n            # MONEY THE VAULT KEPT, MOVED BY HAND',
  ['test_btc_forwarder']),
 ...
 ("the operator alert goes to the deposit's own chat", 'gs_telegram_pager',
  ...
  '                                             or ()):',
  ['test_telegram_pager']),
 ('a failed forward never alerts the operator', 'gs_telegram_pager', '            if out == "failed" or _early in ("kept", "leftover"):\n                # THE RUN DIED', '            if _early in ("kept", "leftover"):\n                # THE RUN DIED', ['test_telegram_pager']),
 ('a forward learned after a restart is not put back on the list', 'gs_telegram_pager', '            if (not e and out == "done"\n                    and phase in ("sent", "unsure", "forwarded", "kept",', '            if (False and out == "done"\n                    and phase in ("sent", "unsure", "forwarded", "kept",', ['test_telegram_pager']),
 ...
 ('not_yet on a recovered entry leaves a looked-at entry with no address',
  ...
  '                if False:',
  ['test_telegram_pager']),
 ('the mined-forward word leaves the closed vocabulary', 'gs_wake_proto.py', '          "delayed", "returned", "forwarded", "kept", "leftover",\n', '          "delayed", "returned", "kept", "leftover",\n', ['test_wake_doorbell', 'test_wake_agent']),
 ...
 ('the pager renders a sent forward as a rehearsal', 'gs_telegram_pager',
  ...
  '                if False:',
  ['test_telegram_pager']),
 # ---- stage 4: the deposit the client makes (unique address, no note) ----
 ('a third plaintext-slip shape is accepted', 'gs_wake_proto.py',
  'PLAIN_SHAPES = (frozenset(PLAIN_FIELDS), frozenset(PLAIN_FIELDS) - {"m"})',
  'PLAIN_SHAPES = (frozenset(PLAIN_FIELDS), frozenset(PLAIN_FIELDS) - {"m"},\n                frozenset(PLAIN_FIELDS) - {"m", "d"})',
  ['test_plain_slip']),
 ('unused() calls an address with history fresh', 'gs_btc_broadcast.py',
  '    return not entries',
  '    return True',
  ['test_btc_broadcast']),
 ('a used address is issued: the freshness answer is ignored', 'gs_wake_agent',
  '        if fresh:\n            if step:',
  '        if True:\n            if step:',
  ['test_wake_agent']),
 ...
 ('the recovery gap is one wider than declared', 'gs_wake_agent',
  ...
  '    for step in range(int(proto.BTC_INDEX_GAP) + 2):',
  ['test_wake_agent']),
 ("the plain slip carries the pairs file's SHARED inbound instead of the derived address",
  'gs_wake_agent',
  '            body["d"] = _btc_address(key, _bi)',
  '            body["d"] = str(pair.get("deposit"))',
  ['test_wake_agent']),
 ('the memo travels on the intake', 'gs_wake_agent',
  '            body["d"] = _btc_address(key, _bi)\n            body["h"] = handle',
  '            body["d"] = _btc_address(key, _bi)\n            body["h"] = handle\n            body["m"] = str(pair.get("memo"))',
  ['test_wake_agent']),
 ('a BTC-intake record is sealed for a delivery machine', 'gs_wake_agent',
  '            integrity_log("wake", "slip_not_sealed_btc")\n            return ""',
  '            integrity_log("wake", "slip_not_sealed_btc")',
  ['test_wake_agent']),
 ('the forwardable floor is removed', 'gs_wake_agent',
  '            if int(_amt) < _floor:',
  '            if False:',
  ['test_wake_agent']),
 # THE FLOOR AND THE CAP DISAGREED AT THE FLOOR (STAGE5_PLAN.md 3.3): a
 # floor built from the dust rule alone accepted deposits the forwarder then
 # refused as fee_eats_deposit whenever fees were at the ceiling.
 ('the floor forgets the fee cap', 'gs_btc_tx.py',
  '    return max(min_send_sat + fee, by_cap_sat)',
  '    return min_send_sat + fee',
  ['test_btc_tx', 'test_btc_forwarder', 'test_wake_agent']),
 ('the floor forgets the fee allowance', 'gs_btc_tx.py',
  '    return max(min_send_sat + fee, by_cap_sat)',
  '    return max(min_send_sat, by_cap_sat)',
  ['test_btc_tx', 'test_btc_forwarder']),
 ('pairing accepts --btc-xpub without --deposit-in-chat', 'gs_wake_keys',
  '    if args.btc_xpub and not args.deposit_in_chat:',
  '    if False:',
  ['test_wake_agent']),
 ('the forwarder says nothing about what it saw when nothing settled',
  'btc_forwarder',
  '        write_status(args.outfile, picture["state"])\n',
  '',
  ['test_btc_forwarder']),
 ('money still confirming is reported as "not yet"', 'gs_wake_agent',
  '_FORWARD_STATUS_PHASE = {"not_seen": "not_yet", "seen": "arriving",',
  '_FORWARD_STATUS_PHASE = {"not_seen": "not_yet", "seen": "not_yet",',
  ['test_wake_agent']),
 ('a forward that found nothing settled is reported as FAILED', 'gs_wake_agent',
  '            _fw = _forward_status_word(artifact_dir, handle)\n            if _fw:\n',
  '            _fw = _forward_status_word(artifact_dir, handle)\n            if False:\n',
  ['test_wake_agent']),
 ('a forward that found nothing settled is marked as a rehearsal', 'gs_wake_agent',
  '            and (artifact_dir / f"btc_forward_{handle}.json").is_file():',
  '            and True:',
  ['test_wake_agent']),
 ('a status file with a second field is read as a word', 'gs_wake_agent',
 ...
 ('pairing accepts a SENDING pair without the THORNode cross-check',
  'gs_wake_keys',
  '    if args.allow_btc_broadcast and not args.thornode:',
  '    if False:',
  ['test_wake_agent']),
 ('the forwarder sends without the THORNode cross-check', 'btc_forwarder',
  '    if args.broadcast and not args.thornode:',
  '    if False:',
  ['test_btc_forwarder']),
 ('the account number is not handed to the signer', 'gs_wake_agent',
  '            env_extra["GS_BTC_ACCOUNT"] = str(\n'
  '                _btc_setting(key, "btc_account", 0, 0, 2 ** 31 - 1))',
  '            env_extra["GS_BTC_ACCOUNT"] = "0"',
  ['test_wake_agent']),
 ('the signer derives account 0 whatever it was told', 'btc_forwarder',
  '        account = account_for(words, passphrase, args.network, xpub,\n'
  '                              account_no=account_no)',
  '        account = account_for(words, passphrase, args.network, xpub,\n'
  '                              account_no=0)',
  ['test_btc_forwarder']),
 # Step 4: a fee refusal is a word, an emptied address is read, a foreign
 # spend is never called ours.
 ('a fee refusal is a dead end: no delayed word', 'btc_forwarder',
  '        if e.kind in DELAY_KINDS:',
  '        if False:',
  ['test_btc_forwarder']),
 ('a deposit that can never be sent on is answered delayed, not short',
  'btc_forwarder',
  '        return watch.STATE_SEEN\n    return STATUS_SHORT',
  '        return watch.STATE_SEEN\n    return STATUS_DELAYED',
  ['test_btc_forwarder']),
 ('short or delayed starts the band at 1 sat/vB: outputs that are dust '
  'at every rate this pair pays read delayed', 'btc_forwarder',
  '    return forwards_at(utxos, min_conf, floor, exclude, op_return_limit,',
  '    return forwards_at(utxos, min_conf, 1, exclude, op_return_limit,',
  ['test_btc_forwarder']),
 ('short or delayed counts the inputs a listed forward of ours spent',
  'btc_forwarder',
  '    band = (floor, ceiling, exclude, op_return_limit, min_send_sat)',
  '    band = (floor, ceiling, (), op_return_limit, min_send_sat)',
  ['test_btc_forwarder']),
 ('short or delayed ignores the fee guards: any selection reads as a '
  'forward', 'btc_forwarder',
  '    return send_refusal(sum(u["value"] for u in chosen), fee,\n'
  '                        min_send_sat) is None',
  '    return True',
  ['test_btc_forwarder']),
 ('money still confirming is short: the Pi never starts it again',
  'btc_forwarder',
  '    if forwards_in_band(whole, min_conf, *band):\n        return watch.STATE_SEEN',
  '    if False:\n        return watch.STATE_SEEN',
  ['test_btc_forwarder']),
 ('money still confirming is weighed at its present depth',
  'btc_forwarder',
  '    whole = [dict(u, confirmations=max(int(u["confirmations"]),\n'
  '                                       int(min_conf), 1)) for u in utxos]',
  '    whole = [dict(u) for u in utxos]',
  ['test_btc_forwarder']),
 ('a replacement refused on today\'s fee reads short while the original '
  'stands', 'btc_forwarder',
  '    if replacing:\n        return STATUS_DELAYED',
  '    if False:\n        return STATUS_DELAYED',
  ['test_btc_forwarder']),
 ('the smallest forward ThorChain takes is refused at exactly the '
  'minimum', 'btc_forwarder',
  '    if send < min_send_sat:\n        return ("below_minimum",',
  '    if send <= min_send_sat:\n        return ("below_minimum",',
  ['test_btc_forwarder']),
 ('a reconciliation\'s own returned/delayed is answered from the plan',
  'gs_wake_agent',
  '            if _word in ("returned", "delayed"):\n                return _word',
  '            if False:\n                return _word',
  ['test_wake_agent']),
 ('a reconciliation\'s short over a moved plan reaches the phone as short',
  'gs_wake_agent',
  '            if _word in ("returned", "delayed"):\n                return _word',
  '            if _word:\n                return _word',
  ['test_wake_agent']),
 ...
 ('a superseded plan reads as never sent', 'gs_wake_agent',
  ...
  '        if False:',
  ['test_wake_agent']),
 ('an unreachable re-send overwrites the earlier send\'s outcome',
  'btc_forwarder',
  '    plan["resend_outcome"] = outcome\n',
  '    plan["resend_outcome"] = outcome\n    plan["broadcast_outcome"] = outcome\n',
  ['test_btc_forwarder']),
 ('the forwarded look ignores a delayed wait', 'gs_telegram_pager',
  '                if time.time() < float(e.get("retry_after") or 0.0):\n'
  '                    return\n'
  '                if not self._btc_can_start(background=True):',
  '                if not self._btc_can_start(background=True):',
  ['test_telegram_pager']),
 ('a hand move the look has not seen is signed over again', 'btc_forwarder',
  '                for i in s["inputs"]} | _moved_ins',
  '                for i in s["inputs"]}',
  ['test_btc_forwarder']),
 ('a stop after the look still sends', 'btc_forwarder',
  '    if shutdown_requested():\n        integrity_log("forward", "stopped_before_send")',
  '    if False:\n        integrity_log("forward", "stopped_before_send")',
  ['test_btc_forwarder']),
 ('a stop during the history read still re-sends', 'btc_forwarder',
  '    stop_before_send("re-sending")\n',
  '',
  ['test_btc_forwarder']),
 ('the seen wait is not told about a stop', 'btc_forwarder',
  '                                avoid=_vouchers_not(sub), stop=shutdown_requested,\n                                distrust=_distrusted(\n',
 ...
 ('a dust flood makes a forward no node relays', 'btc_forwarder',
  ...
  '    while False:',
  ['test_btc_forwarder']),
 ('the cap keeps the smallest outputs, not the largest', 'btc_forwarder',
  '                    key=lambda u: -int(u["value"]))\n    k = len(others)',
  '                    key=lambda u: int(u["value"]))\n    k = len(others)',
  ['test_btc_forwarder']),
 ('a superseded plan with unsettled money back is done', 'btc_forwarder',
  '        if unsettled_new:\n            _k = _kept(unsettled_new, False)',
  '        if False:\n            _k = _kept(unsettled_new, False)',
  ['test_btc_forwarder']),
 ('the superseder is the newest of ours, over any outpoints', 'btc_forwarder',
  '        newest = max(_over, key=',
  '        newest = max((s for s in spends if s["txid"] in ours), key=',
  ['test_btc_forwarder']),
 ('a junk fee estimate reads as no estimate', 'gs_btc_watch.py',
  '        if sat is None:\n            raise BtcWatchError("electrum: bad fee estimate")',
  '        if sat is None:\n            return None',
  ['test_btc_watch']),
 ('a leader with no estimate ends the look', 'gs_btc_watch.py',
  '        if fee_blocks is not None and picture["fee_sat_vb"] is None:\n            kept = picture\n            continue',
  '        if False:\n            kept = picture\n            continue',
  ['test_btc_watch']),
 ('the fallback servers are asked the whole look', 'gs_btc_watch.py',
  '                if kept is not None:\n                    fee = e.estimate_fee(fee_blocks)',
  '                if False:\n                    fee = e.estimate_fee(fee_blocks)',
  ['test_btc_watch']),
 ('an output index wider than four bytes is taken', 'gs_btc_watch.py',
  '        if vout > 0xFFFFFFFF:\n            raise BtcWatchError("electrum: bad listunspent entry (fields)")',
  '        if False:\n            raise BtcWatchError("electrum: bad listunspent entry (fields)")',
  ['test_btc_watch']),
 ('a huge finite timeout is taken', 'gs_btc_watch.py',
  'or not 0 < timeout <= MAX_TIMEOUT:',
  'or not 0 < timeout:',
  ['test_btc_watch']),
 ('the passphrase is not NFKD-normalised', 'gs_btc_tx.py',
  '        words, unicodedata.normalize("NFKD", str(passphrase or "")))',
  '        words, str(passphrase or ""))',
  ['test_btc_tx']),
 ('a deposit address is issued without the constant-time backend', 'gs_wake_agent',
  '    try:\n        _btx.require_native()\n    except _btx.BtcTxError:\n        integrity_log("wake", "btc_backend_missing")',
  '    try:\n        pass\n    except _btx.BtcTxError:\n        integrity_log("wake", "btc_backend_missing")',
  ['test_wake_agent']),
 ('the forwarder looks and quotes before asking for the backend', 'btc_forwarder', '    if not args.plan_only and not args.reconcile:\n        try:\n            btx.require_native()', '    if False:\n        try:\n            btx.require_native()', ['test_btc_forwarder']),
 ('a quote of more Monero than exists is taken', 'btc_forwarder',
  '    if exp > XMR_QUOTE_MAX:',
  '    if False:',
  ['test_btc_forwarder']),
 ('the quantize runs at the default 28 digits', 'btc_forwarder',
  '        _ctx.prec = 80',
  '        _ctx.prec = 28',
  ['test_btc_forwarder']),
 ('routes is indexed before its type is looked at', 'btc_forwarder',
  '    if not isinstance(routes, list) or not routes \\\n',
  '    if not routes \\\n',
  ['test_btc_forwarder']),
 ('NaN and inf seen-wait/interval pass validation', 'btc_forwarder',
  '    if not (math.isfinite(args.seen_wait) and 0 <= args.seen_wait <= 3600',
  '    if False and (math.isfinite(args.seen_wait) and 0 <= args.seen_wait <= 3600',
  ['test_btc_forwarder']),
 ('a NaN or huge --timeout passes validation', 'btc_forwarder',
  '    if not (math.isfinite(args.timeout)\n',
  '    if False and (math.isfinite(args.timeout)\n',
  ['test_btc_forwarder']),
 ('--feerate-sat-vb 0 or negative passes validation', 'btc_forwarder',
  '    if args.feerate_sat_vb is not None and args.feerate_sat_vb < 1:',
  '    if False:',
  ['test_btc_forwarder']),
 ('an --outfile without a .json stem is taken', 'btc_forwarder',
  '    if not str(args.outfile).endswith(".json") \\\n',
  '    if False \\\n',
  ['test_btc_forwarder']),
 ("the limit floor is not raised to the watcher's line", 'btc_forwarder',
  '    floor = max(floor, worst_units)\n',
  '',
  ['test_btc_forwarder']),
 ("the jitter's room ignores the watcher's line", 'btc_forwarder',
  '    _jcap = max(0, min(LIMIT_JITTER_MAX, floor // 100, floor - worst_units))',
  '    _jcap = max(0, min(LIMIT_JITTER_MAX, floor // 100))',
  ['test_btc_forwarder']),
 ('a jitter that ignores its cap is trusted', 'btc_forwarder',
  '    floor = max(1, floor - min(max(0, int(LIMIT_JITTER(_jcap))), _jcap))',
  '    floor = max(1, floor - int(LIMIT_JITTER(_jcap)))',
  ['test_btc_forwarder']),
 ('the funding of kept outputs is not kept in the history', 'btc_forwarder',
  '    _keep |= set(_extra[:KEEP_EXTRA_MAX])\n',
  '    pass\n',
  ['test_btc_forwarder']),
 ('a pin mismatch on the Pi is swallowed', 'gs_telegram_pager',
  '        if _pin and not getattr(self, "_pin_warned", False):',
  '        if False:',
  ['test_telegram_pager']),
 ('a pin mismatch is said on every tick', 'gs_telegram_pager',
  '            self._pin_warned = True\n',
  '',
  ['test_telegram_pager']),
 ('a stop raised before the seen wait still polls every server', 'gs_btc_broadcast.py',
  '    while True:\n        if stop is not None and stop():\n            break\n        polls += 1',
  '    while True:\n        polls += 1',
  ['test_btc_broadcast']),
 ('a stop mid-poll still fails over to the other servers', 'gs_btc_broadcast.py',
  '        if stop is not None and stop():\n            break\n        transport = make(host, port, pin)',
  '        transport = make(host, port, pin)',
  ['test_btc_broadcast']),
 ('a reconciliation that signs does not ask for the backend', 'btc_forwarder',
  '        # before the seed is taken (see the gate above).\n        try:\n            btx.require_native()',
  '        # before the seed is taken (see the gate above).\n        try:\n            pass',
  ['test_btc_forwarder']),
 ('returned money still confirming reads seen, dropped over a moved plan', 'btc_forwarder',
  '            if _word == watch.STATE_SEEN and why == "returned":',
  '            if False:',
  ['test_btc_forwarder']),
 ('the limit floor is raised to the quantized worst figure', 'btc_forwarder',
  '    worst_units = min(\n        int((expected_xmr * (1 - ARRIVAL_TOLERANCE) * Decimal(10 ** 8))\n            .to_integral_value(rounding=ROUND_CEILING)),\n        int(expected_xmr * Decimal(10 ** 8)))',
  '    worst_units = int((max(worst_xmr, expected_xmr * (1 - ARRIVAL_TOLERANCE))\n                       * Decimal(10 ** 8)).to_integral_value(\n                           rounding=ROUND_CEILING))',
  ['test_btc_forwarder']),
 ("the cap drops a replacement's committed inputs", 'btc_forwarder',
  '    keep = {id(u) for u in must}',
  '    keep = set()',
  ['test_btc_forwarder']),
 ('a keyfile switch is coerced: "false" turns a spending job ON', 'gs_wake_agent',
  '    v = key.get(name, False)\n    if not isinstance(v, bool):\n        raise Refused(code,',
  '    v = key.get(name, False)\n    if False:\n        raise Refused(code,',
  ['test_wake_agent']),
 ('the withdraw gate reads its switch loosely again', 'gs_wake_agent',
  '            _allowed = _key_switch(key, "allow_withdraw",\n                                   "withdraw_config_malformed")',
  '            _allowed = key.get("allow_withdraw")',
  ['test_wake_agent']),
 ('a band with no rates in it carries a forward', 'btc_forwarder',
  '    if floor > ceiling:\n        return False\n    return forwards_at',
  '    if False:\n        return False\n    return forwards_at',
  ['test_btc_forwarder']),
 ('the cut is never made: outputs between the dust line and the fifth sink the forward', 'btc_forwarder',
  '    if min_send_sat is not None and (must or k):',
  '    if False:',
  ['test_btc_forwarder']),
 ('main() spends the uncut choice', 'btc_forwarder',
  '            args.op_return_max_bytes, args.min_send_sat)',
  '            args.op_return_max_bytes)',
  ['test_btc_forwarder']),
 ('the word is decided on the uncut choice, not what main() would spend', 'btc_forwarder',
  '    chosen, _, _, _ = spendable(utxos, min_conf, rate, exclude,\n                                op_return_limit, min_send_sat)',
  '    chosen, _, _, _ = spendable(utxos, min_conf, rate, exclude,\n                                op_return_limit)',
  ['test_btc_forwarder']),
 ('the cut takes the shortest passing run, not the longest', 'btc_forwarder',
  '        for j in range(k, -1, -1):\n            if not must and j == 0:\n                break',
  '        for j in range(0 if must else 1, k + 1):\n            if False:\n                break',
  ['test_btc_forwarder']),
 ('the cut ignores the guards: nothing is ever cut', 'btc_forwarder',
  '            if send_refusal(_base + _sums[j], _fee, min_send_sat) is None:',
  '            if True:',
  ['test_btc_forwarder']),
 ('the cut forgets what the `must` inputs carry', 'btc_forwarder',
  '        _base = sum(int(u["value"]) for u in must)',
  '        _base = 0',
  ['test_btc_forwarder']),
 ('--seen-interval over an hour passes validation', 'btc_forwarder',
  '            and 0 < args.seen_interval <= 3600):',
  '            and 0 < args.seen_interval):',
  ['test_btc_forwarder']),
 ('an --outfile named only .json is taken', 'btc_forwarder',
  'str(args.outfile).endswith(("/.json", ".status.json",',
  'str(args.outfile).endswith((".status.json",',
  ['test_btc_forwarder']),
 ('a delayed over a kept plan is said instead of kept', 'gs_wake_agent',
  '            if _forward_kept(artifact_dir, handle, reader):\n                return "kept"\n            # THIS RUN\'S OWN WORD NEXT',
  '            if False:\n                return "kept"\n            # THIS RUN\'S OWN WORD NEXT',
  ['test_wake_agent']),
 ('the seal proves the seed on the variable-time curve', 'gs_wake_agent',
  '                _btx.require_native()\n            except _btx.BtcTxError:\n                raise _NoNativeBackend()',
  '                pass\n            except _btx.BtcTxError:\n                raise _NoNativeBackend()',
  ['test_wake_agent']),
 ("a returned answer to this end's own start has no early wait", 'gs_telegram_pager',
  '                if _was_forwarding:\n                    # ...AND IF THIS END STARTED IT, THE EARLY WAIT',
  '                if False:\n                    # ...AND IF THIS END STARTED IT, THE EARLY WAIT',
  ['test_telegram_pager']),
 ("a delayed after the forwarded look's start leaves the entry seen", 'gs_telegram_pager',
  '                if _was_forwarding and _from_fwd:',
  '                if False:',
  ['test_telegram_pager']),
 ('a delayed on a kept entry leaves its taps due to the forward', 'gs_telegram_pager',
  '                    if e["state"] == "kept":\n                        # ...and a kept one',
  '                    if False:\n                        # ...and a kept one',
  ['test_telegram_pager']),
 ('a huge integer fee estimate overflows out of the look', 'gs_btc_watch.py',
  '                (isinstance(r, int) and r <= 0)\n                or (isinstance(r, float) and math.isfinite(r) and r <= 0)):',
  '                isinstance(r, (int, float)) and math.isfinite(r) and r <= 0):',
  ['test_btc_watch']),
 ("a leader's malformed estimate hands the next server the scripthash", 'gs_btc_watch.py',
  '                    except BtcWatchError:\n                        picture["fee_sat_vb"] = None',
  '                    except ZeroDivisionError:\n                        picture["fee_sat_vb"] = None',
  ['test_btc_watch']),
 ('the mnemonic is not NFKD-normalised', 'gs_btc_tx.py',
  '    words = " ".join(unicodedata.normalize("NFKD", str(mnemonic)).split())',
  '    words = " ".join(str(mnemonic).split())',
  ['test_btc_tx']),
 ("returned money no rate carries is the fresh deposit's short", 'btc_forwarder',
  '            elif _word == STATUS_SHORT and why == "returned":',
  '            elif False:',
  ['test_btc_forwarder']),
 ("a leftover is dropped for the moved plan's forwarded", 'gs_wake_agent',
  '            if _word == "leftover" and outcome == "accepted" and (',
  '            if False and outcome == "accepted" and (',
  ['test_wake_agent']),
 ('a leftover is said before the forward on record has mined', 'gs_wake_agent',
  '            if _word == "leftover" and outcome == "accepted" and (\n                    _forward_mined(artifact_dir, handle, reader)):',
  '            if _word == "leftover" and outcome == "accepted" and (\n                    True):',
  ['test_wake_agent']),
 ('a leftover is said with no plan that moved', 'gs_wake_agent',
  '        return "" if _word == "leftover" else _word',
  '        return _word',
  ['test_wake_agent']),
 ('a leftover answer holds nothing: the same refused forward every window', 'gs_telegram_pager',
  '                e["word"] = "leftover"\n                self._btc_unhold(e)\n                if not _own:',
  '                e["word"] = "leftover"\n                self._btc_unhold(e)\n                return\n                if not _own:',
  ['test_telegram_pager']),
 ("a leftover answer resets the entry's clock", 'gs_telegram_pager',
  '                if not (_was_forwarding and _from_fwd\n                        and e.get("sent_at")):\n                    e["sent_at"] = time.time()',
  '                if True:\n                    e["sent_at"] = time.time()',
  ['test_telegram_pager']),
 ('the forwarded look ignores a hold on the same money', 'gs_telegram_pager',
  '                    if conf == _held and (',
  '                    if False and (',
  ['test_telegram_pager']),
 ('a hold taken at the next look starts that look instead', 'gs_telegram_pager',
  '                    if _held < 0:\n                        e["held_conf"] = conf\n                        e["leftover_once"] = conf\n                        return',
  '                    if False:\n                        e["held_conf"] = conf\n                        e["leftover_once"] = conf\n                        return',
  ['test_telegram_pager']),
 ('a timed hold is never over', 'gs_telegram_pager',
  '                            or time.time() < float(e["held_until"])):',
  '                            or True):',
  ['test_telegram_pager']),
 ('a timed hold is ignored', 'gs_telegram_pager',
  '                            or time.time() < float(e["held_until"])):',
  '                            or False):',
  ['test_telegram_pager']),
 ('a change in the money does not release the hold', 'gs_telegram_pager',
  '                    if conf != _held:\n                        self._btc_unhold(e)',
  '                    if False:\n                        self._btc_unhold(e)',
  ['test_telegram_pager']),
 ('the start does not record the total it was made for', 'gs_telegram_pager',
  '                e["start_conf"] = conf\n                e["start_at"] = self._btc_look_stamp(e)',
  '                e["start_at"] = self._btc_look_stamp(e)',
  ['test_telegram_pager']),
 ("a bare forwarded to this end's own start is not held", 'gs_telegram_pager',
  '            if out == "done" and phase == "forwarded" and _own:',
  '            if False:',
  ['test_telegram_pager']),
 ('the bare-forwarded hold does not double', 'gs_telegram_pager',
  '                        self.btc_recheck_s) * float(2 ** _n)',
  '                        self.btc_recheck_s) * float(2)',
  ['test_telegram_pager']),
 ('a forward that went out leaves the hold in place', 'gs_telegram_pager',
  '                e.pop("fee_tries", None)\n                self._btc_unhold(e)\n                return\n            if out == "done" and phase == "leftover":',
  '                e.pop("fee_tries", None)\n                return\n            if out == "done" and phase == "leftover":',
  ['test_telegram_pager']),
 ('a leftover after a restart is not learned back', 'gs_telegram_pager',
  '                    and phase in ("sent", "unsure", "forwarded", "kept",\n                                  "leftover")',
  '                    and phase in ("sent", "unsure", "forwarded", "kept")',
  ['test_telegram_pager']),
 ("after a leftover the next ask is the forward's, not the XMR side's", 'gs_telegram_pager',
  '            if out == "done" and phase in ("sent", "forwarded", "leftover"):',
  '            if out == "done" and phase in ("sent", "forwarded"):',
  ['test_telegram_pager']),
 ('the operator is not told of a leftover', 'gs_telegram_pager',
  '            if out == "failed" or _early in ("kept", "leftover"):',
  '            if out == "failed" or _early == "kept":',
  ['test_telegram_pager']),
 ('/balance does not name the leftover', 'gs_telegram_pager',
  '            _st = ("leftover" if e["state"] == "forwarded"',
  '            _st = ("forwarded" if e["state"] == "forwarded"',
  ['test_telegram_pager']),
 ('leftover is not a word the doorbell knows', 'gs_wake_proto.py',
  '          "delayed", "returned", "forwarded", "kept", "leftover",\n',
  '          "delayed", "returned", "forwarded", "kept",\n',
  ['test_wake_protocol', 'test_telegram_pager']),
 ('a second leftover for the same total is timed again, never for good', 'gs_telegram_pager',
  '                elif _once == _start_conf and not isinstance(_once, bool):',
  '                elif False:',
  ['test_telegram_pager']),
 ('a first leftover holds for good: a lagging vault strands the payment', 'gs_telegram_pager',
  '                    e["held_until"] = _start_at + float(\n                        self.LEFTOVER_WINDOWS) * float(self.btc_recheck_s)\n                    self._btc_look_from(e, _start_at)',
  '                    self._btc_look_from(e, _start_at)',
  ['test_telegram_pager']),
 ("a first leftover's while counts from the answer, not the start's look", 'gs_telegram_pager',
  '                    e["held_until"] = _start_at + float(\n                        self.LEFTOVER_WINDOWS)',
  '                    e["held_until"] = time.time() + float(\n                        self.LEFTOVER_WINDOWS)',
  ['test_telegram_pager']),
 ("a tap's leftover holds whatever the next look sees for good", 'gs_telegram_pager',
  '                    e["held_conf"] = -1\n                    e["held_until"] = time.time() + float(\n                        self.LEFTOVER_WINDOWS) * float(self.btc_recheck_s)',
  '                    e["held_conf"] = -1',
  ['test_telegram_pager']),
 ('the total the next look takes is not the leftover the vault gave', 'gs_telegram_pager',
  '                        e["held_conf"] = conf\n                        e["leftover_once"] = conf\n                        return',
  '                        e["held_conf"] = conf\n                        return',
  ['test_telegram_pager']),
 ('a start from seen reads as one this end did not make', 'gs_telegram_pager',
  '            _own = _was_forwarding and isinstance(',
  '            _own = _was_forwarding and _from_fwd and isinstance(',
  ['test_telegram_pager']),
 ('the start from seen does not record the total it was made for', 'gs_telegram_pager',
  '                    e["start_conf"] = conf\n                    e["start_at"] = time.time()',
  '                    e["start_at"] = time.time()',
  ['test_telegram_pager']),
 ('a start stamped in the future stretches the while', 'gs_telegram_pager',
  '            if not (0.0 < _start_at <= time.time()):\n                _start_at = time.time()',
  '            if False:\n                _start_at = time.time()',
  ['test_telegram_pager']),
 ('the bare-forwarded hold is never for good', 'gs_telegram_pager',
  '                if _n <= int(self.HELD_TIMED_TRIES):',
  '                if True:',
  ['test_telegram_pager']),
 ('the first bare forwarded holds for good', 'gs_telegram_pager',
  '                if _n <= int(self.HELD_TIMED_TRIES):',
  '                if False:',
  ['test_telegram_pager']),
 ("the bare-forwarded while counts from the answer, not the start's look", 'gs_telegram_pager',
  '                    e["held_until"] = _start_at + float(\n                        self.btc_recheck_s)',
  '                    e["held_until"] = time.time() + float(\n                        self.btc_recheck_s)',
  ['test_telegram_pager']),
 ("a bare forwarded resets the forward's clock every time", 'gs_telegram_pager',
  '                if not (_from_fwd and e.get("sent_at")):\n                    e["sent_at"] = time.time()\n                e["state"] = "forwarded"\n                e["word"] = phase\n                e["held_conf"] = _start_conf',
  '                if True:\n                    e["sent_at"] = time.time()\n                e["state"] = "forwarded"\n                e["word"] = phase\n                e["held_conf"] = _start_conf',
  ['test_telegram_pager']),
 ("a bare forwarded to a start from seen keeps the first forward's clock", 'gs_telegram_pager',
  '                if not (_from_fwd and e.get("sent_at")):\n                    e["sent_at"] = time.time()\n                e["state"] = "forwarded"\n                e["word"] = phase\n                e["held_conf"] = _start_conf',
  '                if False:\n                    e["sent_at"] = time.time()\n                e["state"] = "forwarded"\n                e["word"] = phase\n                e["held_conf"] = _start_conf',
  ['test_telegram_pager']),
 ("an entry still owed a look or a start is forgotten on its clock", 'gs_telegram_pager',
  '                           >= proto.DEPOSIT_PLACE_TTL_S\n                           and not self._btc_keep_past_clock(v, _now))]:',
  '                           >= proto.DEPOSIT_PLACE_TTL_S)]:',
  ['test_telegram_pager']),
 ('no slack after a timed hold: forgotten on the tick its ask was due', 'gs_telegram_pager',
  '        _slack = float(self.HOLD_SLACK_WINDOWS) * float(self.btc_recheck_s)',
  '        _slack = 0.0',
  ['test_telegram_pager']),
 ("a failed run of the forwarded look's start goes back to seen (every tick)", 'gs_telegram_pager',
  '                    e["state"] = ("forwarded" if _was_forwarding and _from_fwd\n                                  else "seen")',
  '                    e["state"] = ("seen" if _was_forwarding and _from_fwd\n                                  else "seen")',
  ['test_telegram_pager']),
 ('a forwarded entry is forgotten before its look past the clock', 'gs_telegram_pager',
  '            if _ans < _end and now < _end + float(\n                    self.FINAL_LOOK_WINDOWS) * float(self.btc_recheck_s):\n                return True',
  '            if False:\n                return True',
  ['test_telegram_pager']),
 ('the wait for the look past the clock has no end of its own when looks fail', 'gs_telegram_pager',
  '            if _ans < _end and now < _end + float(\n                    self.FINAL_LOOK_WINDOWS) * float(self.btc_recheck_s):',
  '            if _ans < _end:',
  ['test_telegram_pager']),
 ('the look past the clock is never recorded as answered', 'gs_telegram_pager',
  '                e["look_answered_at"] = time.time()',
  '                pass',
  ['test_telegram_pager']),
 ('a look that failed counts as the look past the clock', 'gs_telegram_pager',
  '                    opened[_h]["looked_at"] = _now',
  '                    opened[_h]["looked_at"] = _now\n                    opened[_h]["look_answered_at"] = _now',
  ['test_telegram_pager']),
 ('an entry with no address waits for a look that never comes', 'gs_telegram_pager',
  '        if _st == "forwarded" and e.get("addr"):',
  '        if _st == "forwarded":',
  ['test_telegram_pager']),
 ("a retry's wait does not keep an entry past its clock", 'gs_telegram_pager',
  '                          ("retry_after", True)):',
  '                          ("retry_after", False)):',
  ['test_telegram_pager']),
 ("a timed hold's ask does not keep an entry past its clock", 'gs_telegram_pager',
  '("held_until", _st == "forwarded"\n                           and e.get("held_conf") is not None)',
  '("held_until", False)',
  ['test_telegram_pager']),
 ("a sent entry's held-back recheck is not owed", 'gs_telegram_pager',
  '        if _st not in ("forwarded", "sent"):\n            return False\n        _slack',
  '        if _st not in ("forwarded",):\n            return False\n        _slack',
  ['test_telegram_pager']),
 ('a sent entry is never kept past its clock', 'gs_telegram_pager',
  '        if _st not in ("forwarded", "sent"):\n            return False\n        try:',
  '        if _st not in ("forwarded",):\n            return False\n        try:',
  ['test_telegram_pager']),
 ('nothing caps what keeps an entry past its clock', 'gs_telegram_pager',
  '        if now >= _end + float(self.OWED_GRACE_S):\n            return False',
  '        if False:\n            return False',
  ['test_telegram_pager']),
 ('what the cap ended is said to nobody', 'gs_telegram_pager',
  '                if self._btc_owed(opened[_h], _now):\n                    _stopped.append',
  '                if False:\n                    _stopped.append',
  ['test_telegram_pager']),
 ('an entry owed nothing is said to have stopped', 'gs_telegram_pager',
  '                if self._btc_owed(opened[_h], _now):\n                    _stopped.append',
  '                if True:\n                    _stopped.append',
  ['test_telegram_pager']),
 ('every forwarded-look start is said, retries and re-asks included', 'gs_telegram_pager',
  '                if not _again:\n                    e["said"].discard("confirmed")',
  '                if True:\n                    e["said"].discard("confirmed")',
  ['test_telegram_pager']),
 ('money that is news past a hold is never said', 'gs_telegram_pager',
  '                if not _again:\n                    e["said"].discard("confirmed")',
  '                if False:\n                    e["said"].discard("confirmed")',
  ['test_telegram_pager']),
 ('a stall or fee retry from the forwarded look is said again', 'gs_telegram_pager',
  '                    _again = (int(e.get("stall_tries") or 0) > 0\n                              or int(e.get("fee_tries") or 0) > 0)',
  '                    _again = False',
  ['test_telegram_pager']),
 ("a hold's one more ask is said again", 'gs_telegram_pager',
  '                        and _held >= 0 and conf == _held:\n                    _again = True',
  '                        and _held >= 0 and conf == _held:\n                    _again = False',
  ['test_telegram_pager']),
 ("a held entry's windows count from wherever its look last was", 'gs_telegram_pager',
  '        if at > _was:\n            e["looked_at"] = float(at)',
  '        if False:\n            e["looked_at"] = float(at)',
  ['test_telegram_pager']),
 ('/balance names a bare forwarded held for good as leftover', 'gs_telegram_pager',
  '            _st = ("leftover" if e["state"] == "forwarded"\n                   and e.get("word") == "leftover"\n',
  '            _st = ("leftover" if e["state"] == "forwarded"\n',
  ['test_telegram_pager']),
 ('/balance keeps the leftover sentence after a change released the hold', 'gs_telegram_pager',
  '                   and e.get("held_conf") is not None else e["state"])',
  '                   else e["state"])',
  ['test_telegram_pager']),
 ('the leftover word is not kept on the entry', 'gs_telegram_pager',
  '                e["word"] = "leftover"\n                self._btc_unhold(e)',
  '                e["word"] = "forwarded"\n                self._btc_unhold(e)',
  ['test_telegram_pager']),
 ("a leftover keeps the old hold's while and count", 'gs_telegram_pager',
  '                e["word"] = "leftover"\n                self._btc_unhold(e)\n                if not _own:',
  '                e["word"] = "leftover"\n                if not _own:',
  ['test_telegram_pager']),
 ('a word this end lacks leaves a hold standing', 'gs_telegram_pager',
  '                # word this end read, and this word it could not.\n                self._btc_unhold(e)',
  '                # word this end read, and this word it could not.\n                pass',
  ['test_telegram_pager']),
 ('kept leaves a hold standing', 'gs_telegram_pager',
  '                e["rechecked_at"] = time.time()\n                e.pop("retry_after", None)\n                e.pop("fee_tries", None)\n                self._btc_unhold(e)\n                return',
  '                e["rechecked_at"] = time.time()\n                e.pop("retry_after", None)\n                e.pop("fee_tries", None)\n                return',
  ['test_telegram_pager']),
 ('returned leaves a hold, and the leftover heard, standing', 'gs_telegram_pager',
  '                e.pop("fee_tries", None)\n                self._btc_unhold(e)\n                if _was_forwarding:',
  '                e.pop("fee_tries", None)\n                if _was_forwarding:',
  ['test_telegram_pager']),
 ('a released hold keeps the leftover it heard: the same money back is for good at once', 'gs_telegram_pager',
  '        for _k in ("held_conf", "held_until", "held_tries", "leftover_once"):',
  '        for _k in ("held_conf", "held_until", "held_tries"):',
  ['test_telegram_pager']),
 ('an emptied address is never read for our own forward', 'btc_forwarder',
  '            if found is not None:\n                return found',
  '            if False:\n                return found',
  ['test_btc_forwarder']),
 ('a foreign spend is reported as our own forward', 'btc_forwarder',
 ...
 ('spends_of reports a funding transaction as a spend', 'gs_btc_broadcast.py',
  ...
  '            if spent or pays:\n                out.append({"txid": e["tx_hash"],',
  ['test_btc_broadcast']),
 ('transaction() takes a transaction that is not the one asked for',
  'gs_btc_broadcast.py',
  '        if tx.txid().hex() != want:',
  '        if False:',
  ['test_btc_broadcast']),
 ...
 ('a hand move of the kept outputs is not recorded (the run after cries '
  ...
  '            if not _ins_all <= set(_seen_moved):\n',
  '            if False:\n',
  ['test_btc_forwarder']),
 ('the moved record is not read when the kept set is built',
  ...
  '        _kept_ops |= set()',
  ['test_btc_forwarder']),
 ('a bump clears the kept mark (the client hears sent about kept money)', 'btc_forwarder',
  '        if stuck is None or not _at_limit:\n            plan.pop("returned_kept", None)\n        # THE PUSH OF AN UNCONFIRMED LISTING',
  '        if True:\n            plan.pop("returned_kept", None)\n        # THE PUSH OF AN UNCONFIRMED LISTING',
  ['test_btc_forwarder']),
 ...
 ('the fee jitter can push a floor deposit into a refusal',
  ...
  '        _room = 10 ** 9',
  ['test_btc_forwarder']),
 ("the memo's limit is 99 times an integer (a formula on the chain)", 'btc_forwarder', '    floor = max(1, floor - min(max(0, int(LIMIT_JITTER(_jcap))), _jcap))', '    floor = max(1, floor - 0)', ['test_btc_forwarder']),
 ('the limit jitter is not capped at a hundredth of the floor', 'btc_forwarder', '    _jcap = max(0, min(LIMIT_JITTER_MAX, floor // 100, floor - worst_units))', '    _jcap = max(0, floor - worst_units)', ['test_btc_forwarder']),
 ('the default fee jitter is constant', 'btc_forwarder',
  '    return secrets.randbelow(max(1, int(bound)))',
  '    return 0',
  ['test_btc_forwarder']),
 ('/balance totals what the looks saw on the chain into the transcript', 'gs_telegram_pager', '            lines.append(f"{_cn}: {self.BTC_STATE_WORDS.get(_st, _st)}")\n        return "\\n".join(lines)', '            lines.append(f"{_cn}: {self.BTC_STATE_WORDS.get(_st, _st)}")\n        return "\\n".join(lines) + "\\nreceived so far: 0.055"', ['test_telegram_pager']),
 ...
 ('an output a plan of ours signed for still counts as kept (a foreign RBF '
  ...
  '    _kept_ops -= set()',
  ['test_btc_forwarder']),
 ('a bump under a raised bound keeps the mark (carried onto the replacement, the agent says kept about money the next forward sends)', 'btc_forwarder',
  '        if stuck is None or not _at_limit:\n            plan.pop("returned_kept", None)\n        # THE PUSH OF AN UNCONFIRMED LISTING',
  '        if stuck is None:\n            plan.pop("returned_kept", None)\n        # THE PUSH OF AN UNCONFIRMED LISTING',
  ['test_btc_forwarder']),
 ...
 ('a hand move leaves the moved output on the kept mark', 'btc_forwarder',
  '            if isinstance(_rk, dict) \\\n                    and _pairs(_rk.get("outpoints")) & _ins_all:\n',
  '            if False:\n',
  ['test_btc_forwarder']),
 ...
 ('the intake pay message is sent once, no retry', 'gs_telegram_pager',
  ...
  '                    _ok = _send_pay()',
  ['test_telegram_pager']),
 ('an address nobody was shown stays on the watch list for two days', 'gs_telegram_pager',
  '                _LAST_MID.failed = True\n                self._btc_forget(h)\n',
  '                _LAST_MID.failed = True\n',
  ['test_telegram_pager']),
 ...
 ('a history over the window is refused outright again (a dust flood jams the deposit for good)', 'gs_btc_broadcast.py',
 ...
 ('the reconciliation asks for no word on a truncated history', 'btc_forwarder',
 ...
 ('the exact intake floor (the fee ceiling in disguise) reaches the chat',
  ...
  '    return _exact',
  ['test_wake_agent']),
 # HOST-PRIVACY PASS: two public events at a fixed offset are a signature.
 ('the bump window is the flag\'s exact number again', 'btc_forwarder',
  '        spends, ours, _est, BUMP_WINDOW(args.bump_after))',
  '        spends, ours, _est, args.bump_after)',
  ['test_btc_forwarder']),
 ('the bump window may be drawn BELOW what the operator set',
  'btc_forwarder',
  '    return base + secrets.randbelow(int(base * BUMP_SPREAD_MAX) + 1)',
  '    return base - secrets.randbelow(int(base * BUMP_SPREAD_MAX) + 1)',
  ['test_btc_forwarder']),
 ('a settled deposit is forwarded on the tick that sees it settle',
  'gs_telegram_pager',
  '                if state == "confirmed" and (_waiting or _holding):',
  '                if state == "confirmed" and _waiting:',
  ['test_telegram_pager']),
 ("the hold is redrawn on every tick (so it never runs out)",
  'gs_telegram_pager',
  '                if state == "confirmed" and not e.get("forward_hold") \\\n'
  '                        and not _waiting:',
  '                if state == "confirmed" and not _waiting:',
  ['test_telegram_pager']),
 ('the shipped default forwards at once (the hold is off unless asked)',
  'gs_telegram_pager',
  '    btc_hold_max_s = 1800',
  '    btc_hold_max_s = 0',
  ['test_telegram_pager']),
 ...
 ('a refusal after M2 is never told to the Pi, which holds its lock for '
  ...
  '            raise _m2e\n',
  ['test_wake_agent']),
 ('--unseal-state writes the records out and leaves the container, so a '
  're-pairing refuses every wake for ever', 'gs_wake_agent',
  '        _gone = secure_delete_or_warn(_old, "the retired sealed records")\n',
  '        _gone = True\n',
  ['test_wake_agent', 'test_wake_endtoend']),
 ('--unseal-state reports opened when the container would not shred',
  'gs_wake_agent',
  '    if not _gone:\n',
  '    if False:\n',
  ['test_wake_agent']),
 ('the pairing goes ahead over a sealed store the new pair can never open',
  'gs_wake_keys',
  '    if _left:\n        sys.exit(_left)\n',
  '    if False:\n        sys.exit(_left)\n',
  ['test_wake_endtoend']),
 ('the pairing does not look for a sealed store', 'gs_wake_keys',
  '    if store.exists():\n',
  '    if False:\n',
  ['test_wake_endtoend']),
 ('the pairing does not look for sealed spend secrets', 'gs_wake_keys',
  '    if secrets.exists():\n',
  '    if False:\n',
  ['test_wake_endtoend']),
 ...
 ('--unseal-key keeps the box on only once it has returned', 'gs_wake_agent',
  ...
  '                code = unseal_key_cli(args)\n                _keep_on = True',
  ['test_wake_agent']),
 ("a rejected re-send's fresh forward signs again over an outpoint our listed forward spends", 'btc_forwarder',
  '            return "resend", plan["tx_hex"], sorted(consumed | set(_kept_ex))',
  '            return "resend", plan["tx_hex"], list(_kept_ex)',
  ['test_btc_forwarder']),
 ("the bump compares today's estimate with the rate aimed at, not the rate paid", 'btc_forwarder',
  '    above = (fee_sat_vb * vs > fee) if _real else (fee_sat_vb > paid)\n',
  '    above = fee_sat_vb > paid if _aimed else False\n',
  ['test_btc_forwarder']),
 ('the history read keeps one deadline, and a dust flood runs it out', 'gs_btc_broadcast.py',
  '                _allow(len(entries))\n',
  '',
  ['test_btc_broadcast']),
 ('our own transactions fall off the history window like any dust', 'gs_btc_broadcast.py',
  '                                if e["tx_hash"] in _keep]',
  '                                if False]',
  ['test_btc_broadcast']),
 ('the reconciliation names none of its own transactions to the history read', 'btc_forwarder',
  '                            keep_txids=sorted(_keep))',
  '                            keep_txids=())',
  ['test_btc_forwarder']),
 ("the recheck waits --btc-recheck, not the forward's own draw", 'gs_telegram_pager',
  '        return max(_w, float(self.btc_recheck_s))',
  '        return float(self.btc_recheck_s)',
  ['test_telegram_pager']),
 ('the recheck window is not drawn: every bump lands on the public grid', 'gs_telegram_pager',
  '        return float(_b + self.rng.randint(0, int(_b * self.RECHECK_SPREAD)))',
  '        return float(_b)',
  ['test_telegram_pager']),
 ...
 ("dust in the mempool under the floor is 'received' and ends the pay message's hold", 'gs_telegram_pager',
  '                elif state == "seen" and conf + unconf < _fl:\n',
  '                elif False:\n',
  ['test_telegram_pager']),
 ('a `short` answer stalls the entry, so a top-up is never sent on', 'gs_telegram_pager',
  '            if out == "done" and phase == "short":\n',
  '            if False:\n',
  ['test_telegram_pager']),
 ('after `short` the same settled sum starts the same forward, a wake a tick', 'gs_telegram_pager',
  '                    if conf <= _short:\n                        state = "seen"',
  '                    if False:\n                        state = "seen"',
  ['test_telegram_pager']),
 ('the forwarder takes a plaintext http:// THORNode', 'btc_forwarder',
  '    if args.thornode and not watch.thornode_url_ok(args.thornode):',
  '    if False:',
  ['test_btc_forwarder']),
 ('the pairing takes a plaintext http:// THORNode', 'gs_wake_keys',
  '    if args.thornode and not _w.thornode_url_ok(args.thornode):',
  '    if False:',
  ['test_wake_endtoend']),
 ('any http:// THORNode passes, onion or not', 'gs_btc_watch.py',
  '    return u.scheme == "http" and host.endswith(".onion")',
  '    return u.scheme in ("http", "https")',
  ['test_btc_forwarder']),
 ('a tap inside the hold starts the forward at once', 'gs_telegram_pager',
  '                if self._btc_holding(_h):\n',
  '                if False:\n',
  ['test_telegram_pager']),
 ('a stale superseded mark survives our own transaction being listed', 'btc_forwarder',
  '        plan.pop("superseded_by", None)\n        plan.pop("superseded_height", None)\n',
  '',
  ['test_btc_forwarder']),
 ("the bump age is measured from the stamp bucket's start, up to 599 s early", 'btc_forwarder',
  '        age -= STAMP_BUCKET_S - 1\n',
  '        pass\n',
  ['test_btc_forwarder']),
 ("a --tor-proxy the look refuses passes the pager's start", 'gs_telegram_pager',
  '            _w._socks_parts(getattr(args, "tor_proxy", ""), "btcwatch:probe")',
  '            pass',
  ['test_telegram_pager']),
 ('an address of another network is watched, and every look fails in silence', 'gs_telegram_pager',
  '        if not self._btc_addr_ok(addr):\n',
  '        if False:\n',
  ['test_telegram_pager']),
 ('a watcher whose every look fails never says so', 'gs_telegram_pager',
  '            if self._look_fail_ticks == int(self.LOOK_FAIL_WARN_TICKS):',
  '            if False:',
  ['test_telegram_pager']),
 ('a deposit first seen already confirmed is held in silence', 'gs_telegram_pager',
  '                    if _was == "not_seen" and "seen" not in e["said"]:\n',
  '                    if False:\n',
  ['test_telegram_pager']),
 ('the Pi looks at every open address each tick, a burst on one clock', 'gs_telegram_pager',
  '            if getattr(self, "btc_spread_looks", False) and snapshot:',
  '            if False:',
  ['test_telegram_pager']),
 ('an address looked at is not rescheduled, so the same one is looked at every tick', 'gs_telegram_pager',
  '                    opened[_h1]["next_look"] = _now + self._btc_look_draw()',
  '                    pass',
  ['test_telegram_pager']),
 ('the fee jitter is drawn over the bound and clamped, weighting the cap', 'btc_forwarder',
  '        _w = max(1, min(int(vsize_bound), int(_room) + 1))',
  '        _w = int(vsize_bound)',
  ['test_btc_forwarder']),
 ("the dust branch lands the send exactly on THORNode's published threshold", 'btc_forwarder',
  '                _more = (max(0, min(int(FEE_JITTER(_spare + 1)), _spare))\n                         if _spare > 0 else 0)',
  '                _more = 0',
  ['test_btc_forwarder']),
 ('a reconstructed plan is never bumped though it knows its rate', 'btc_forwarder',
  '    if not (_real or _aimed):\n        return False',
  '    if not _aimed:\n        return False',
  ['test_btc_forwarder']),
 ('the Tor check handed the bare proxy runs on the default circuit', 'gs_common.py',
  '        data = _verify_tor_once(_own_stream(proxy, "tor:check"))',
  '        data = _verify_tor_once(proxy)',
  ['test_listed_bugs']),
 ("the price oracle shares the Tor check's circuit", 'gs_common.py',
  '        p = fetch(CG_PRICE_URL, _own_stream(proxies, "oracle"))',
  '        p = fetch(CG_PRICE_URL, proxies)',
  ['test_listed_bugs']),
 ('a sending forward takes an unpinned clearnet Electrum server', 'btc_forwarder',
  '    if (args.broadcast or args.reconcile) and not all(',
  '    if False and not all(',
  ['test_btc_forwarder']),
 ('a broadcasting pair is paired with an unpinned clearnet Electrum server', 'gs_wake_keys',
  '                and not _w.server_authenticated(_srv):',
  '                and False:',
  ['test_wake_endtoend']),
 ('any server counts as authenticated', 'gs_btc_watch.py',
 ...
 ('a shell-exported GS_BTC_ENTRY reaches a receive run', 'gs_console',
  ...
  '        return ()\n',
  ['test_console']),
 # ---- wt17c: pairing keys, the memo limit, the record of what was sent --
 ('the vault keeps one pairing key for every connection', 'gs_wake_keys',
  ...
  '                my_pub = sk.public_key.encode()\n',
  ['test_wake_endtoend']),
 ('a key that does not match its commitment is an ordinary pairing error',
  'gs_wake_proto.py',
  '        raise PairTampered(\n',
  '        raise WakeError(\n',
  ['test_wake_endtoend']),
 ('the vault listens on after a tampered reveal', 'gs_wake_keys',
  '            except proto.PairTampered as e:\n',
  '            except ZeroDivisionError as e:\n',
  ['test_wake_endtoend']),
 ('the memo limit pattern is unbounded again', 'btc_forwarder',
  '_LIMIT_RE = re.compile(r"^[0-9]{1,20}(?:[eE][0-9]{1,2})?\\Z")\n',
  '_LIMIT_RE = re.compile(r"^[0-9]+(?:[eE][0-9]+)?\\Z")\n',
  ['test_btc_forwarder']),
 ('a spend in the history is adopted as ours by its copyable memo',
  'btc_forwarder',
  '        if mine and str(s["txid"]).lower() in _sent_here:\n',
  '        if mine:\n',
  ['test_btc_forwarder']),
 ('an emptied address is reconstructed as ours by its copyable memo',
  'btc_forwarder',
  '    if not (sent_here and memo_ours):\n',
  '    if not memo_ours:\n',
  ['test_btc_forwarder']),
 ('a forward is sent before its txid is recorded', 'btc_forwarder',
  ...
  '        integrity_log("forward", "sending")\n',
  ['test_btc_forwarder']),
 ('a record that cannot be written still sends', 'btc_forwarder',
  '        raise Refused("signed_record_failed",\n'
  '                      f"the record of what',
  '        return None and Refused("signed_record_failed",\n'
  '                      f"the record of what',
  ['test_btc_forwarder']),
 ('the sent record grows without bound', 'btc_forwarder',
 ...
 ('every name of a hard-linked file is counted, whatever the sweep makes of it',
  'paranoia_mode',
  '                _w = sweep_refusal(_r, _m, owners)\n'
  '                if _w is not None and _w[2] != "hard_link":\n'
  '                    continue\n',
  '                _w = None\n',
  ['test_opsec_guarantees']),
 ('a hard-linked file skips the content check', 'gs_common.py',
  '    if why:\n        return (why, False, "content")\n    # LAST, so',
  '    if why and st.st_nlink == 1:\n        return (why, False, "content")\n    # LAST, so',
  ['test_opsec_guarantees']),
 ('a wipe under sudo searches root\'s home only', 'gs_common.py',
  '    _su = sudo_home()\n',
  '    _su = None\n',
  ['test_opsec_guarantees']),
 ('the fee bound ignores the inputs the plan counted', 'airgap_tx_signer',
  '    _cap = _per * fee_scale_for_inputs(_tx.get("inputs"))\n',
  '    _cap = _per\n',
  ['test_signer_schema']),
 ('--max-fee-xmr is parsed and ignored', 'airgap_tx_signer',
  '    if max_fee is not None:\n        _cap = Decimal(max_fee)\n',
  '    if False:\n        _cap = Decimal(max_fee)\n',
  ['test_signer_schema']),
 ('--max-fee-xmr takes any amount', 'airgap_tx_signer',
  '    if not v.is_finite() or v <= 0 or v > FEE_CAP_PER_TX * 10:\n',
  '    if not v.is_finite() or v <= 0:\n',
  ['test_signer_schema']),
 ('a plan\'s input count is not validated', 'airgap_tx_signer',
  '        if _nin is not None and (isinstance(_nin, bool)\n',
  '        if False and (isinstance(_nin, bool)\n',
  ['test_signer_schema']),
 ('the exact-consume re-price is two percent at any priority',
  'airgap_tx_signer',
  '                fee = fee + max(1, min(fee // 50, _CONSUME_DUST_PICO // 4))\n',
  '                fee = fee + max(fee // 50, 1)\n',
  ['test_signer_schema']),
 ('a build the signer will refuse is settled for', 'airgap_tx_signer',
  '            if result is not None and residue <= _CONSUME_DUST_PICO:\n',
  '            if result is not None:\n',
  ['test_signer_schema']),
 ('the fee grows by a tenth as much per input', 'gs_common.py',
  'FEE_PER_INPUT_MULT = Decimal("0.335")\n',
  'FEE_PER_INPUT_MULT = Decimal("0.0335")\n',
  ['test_dag_entry', 'test_signer_schema']),
 ('a dusted veil reserves a one-input fee', 'GhostSpiral',
  '            bal - sum((veil_fee_reserve(fee_xmr, _v.get("inputs"))\n',
  '            bal - sum((hop_fee_reserve(fee_xmr)\n',
  ['test_dag_entry']),
 ('a dusted veil does not carry its input count', 'GhostSpiral',
  '        if planned_inputs(_n_inputs) is not None:\n'
  '            _veil["inputs"] = planned_inputs(_n_inputs)\n',
  '        if False:\n'
  '            _veil["inputs"] = planned_inputs(_n_inputs)\n',
  ['test_dag_entry']),
 ('a dusted exit sweep does not carry its input count', 'GhostSpiral',
  '        if _nin is not None:\n            tx["inputs"] = _nin\n',
  '        if False:\n            tx["inputs"] = _nin\n',
  ['test_exit_withdraw']),
 ('the peel plan is not levelled', 'GhostSpiral',
  '            return level_peel_last(\n'
  '                trial, peel_last_extra(trial, bal, fee_xmr, surplus_extra),\n'
  '                min_exit_fundable(fee_xmr, dag_enabled)), frac\n',
  '            return trial, frac\n',
  ['test_units']),
 ('the peel surplus forgets the fees the hops spend', 'GhostSpiral',
  '    spent = fee_xmr * fee_mult_for_inputs(1) * Decimal(n)\n',
  '    spent = Decimal(0)\n',
  ['test_units']),
 ('the levelled last share can go under the floor', 'GhostSpiral',
  '    move = min(extra, max(Decimal(0), out[-1] - floor)).quantize(',
  '    move = min(extra, out[-1]).quantize(',
  ['test_units']),
 ('every receive wallet shares one quote file', 'gs_console',
  '    return f"thor_pairs_{stem}.json" if stem else DEFAULT_PAIRS_FILE\n',
  '    return DEFAULT_PAIRS_FILE\n',
  ['test_console']),
 ('a quote file with no swap for this address says to set 0 swaps',
  'gs_console',
  '                f"\'Total XMR expected\'." if _np == 0 else\n',
  '                f"\'Total XMR expected\'." if False else\n',
  ['test_console']),
 ('a typed total does not stand without a quote file', 'receive_watch',
  '            if args.expect_xmr is None:\n'
  '                sys.exit(f"[!] {e}")\n'
  '            allp, _pairs_note = [], f"{e}"\n',
  '            sys.exit(f"[!] {e}")\n',
  ['test_receive_watch']),
 ('a typed total does not stand beside another address\'s quote',
  'receive_watch',
  '            if args.expect_xmr is None:\n'
  '                sys.exit(\n'
  '                    f"[!] None of the',
  ...
  '                    f"[!] None of the',
  ['test_receive_watch']),
 # ---- wt17e: each chunk pays its own veil --------------------------------
 ('chunks are sized by what arrived, not what their carrier will hold',
  'GhostSpiral',
  '            [max(DUST_XMR, _u - _v) for _u, _v in zip(unlocked, _veil_each)],\n',
  '            unlocked,\n',
  ['test_dag_entry']),
 ('chunk sizing charges every veil a one-input fee', 'GhostSpiral',
  '        _veil_each = ([veil_fee_reserve(fee_xmr, _n) for _n in _vin]\n',
  '        _veil_each = ([hop_fee_reserve(fee_xmr) for _n in _vin]\n',
  ['test_dag_entry']),
 # ---- wt17f: the relay reads only its own directory ----------------------
 ('the relay reads whatever path the manifest names', 'broadcast_signed_xmr',
  '        path = mdir / raw.name\n',
  '        path = raw if raw.exists() else mdir / raw.name\n',
  ['test_broadcast']),
 ('the relay follows a symlinked blob', 'broadcast_signed_xmr',
  '    fd = os.open(str(path), os.O_RDONLY | getattr(os, "O_NOFOLLOW", 0))\n',
  '    fd = os.open(str(path), os.O_RDONLY)\n',
  ['test_broadcast']),
 # ---- wt17g: the review of the 17c round ---------------------------------
 ('an external number of any magnitude is parsed', 'gs_common.py',
  '    if not v.is_finite() or not decimal_in_range(v):\n        return default\n',
  '    if not v.is_finite():\n        return default\n',
  ['test_btc_forwarder']),
 ('two wallet names that clean to one share a quote file', 'gs_console',
  '    if stem and stem != raw:\n',
  '    if False:\n',
  ['test_console']),
 # ---- wt17h: the host-privacy review and its self-review --------------------
 ('the wallet-rpc client follows redirects', 'gs_common.py',
  '        _sess.max_redirects = 0\n',
  '',
  ['test_ipleak']),
 ('the wallet-rpc client takes proxies from the environment', 'gs_common.py',
  '        _sess.trust_env = False\n',
  '',
  ['test_ipleak']),
 ('a loopback call leaves ALL_PROXY open', 'gs_common.py',
  '    keys = ["http", "https", "all"]\n',
  '    keys = ["http", "https"]\n',
  ['test_ipleak']),
 ('a loopback call leaves a host-named proxy variable open', 'gs_common.py',
  '        keys += [f"http://{host}", f"https://{host}", f"all://{host}"]\n',
  '        pass\n',
  ['test_ipleak']),
 ('the fee estimate reads its loopback proxy from the environment', 'gs_common.py',
  '    use_proxies = loopback_proxies(daemon_url)\n    if host not in _LOCALHOST_NAMES:\n        if not proxies:\n            return {}\n',
  '    use_proxies = None\n    if host not in _LOCALHOST_NAMES:\n        if not proxies:\n            return {}\n',
  ['test_ipleak']),
 ('the relay-egress probe reads its loopback proxy from the environment', 'gs_common.py',
  '    use_proxies = loopback_proxies(daemon_url)\n    if host not in _LOCALHOST_NAMES:\n        if not proxies:\n            out["detail"]',
  '    use_proxies = None\n    if host not in _LOCALHOST_NAMES:\n        if not proxies:\n            out["detail"]',
  ['test_ipleak']),
 ('the relay submit reads its loopback proxy from the environment', 'broadcast_signed_xmr',
  '        proxies = loopback_proxies(url)\n',
  '        proxies = None\n',
  ['test_ipleak']),
 ("the console's wallet check takes proxies from the environment", 'gs_console',
  "_s=requests.Session(); _s.trust_env=False",
  "_s=requests.Session()",
  ['test_ipleak']),
 ("the console's wallet check follows redirects", 'gs_console',
  "r=_s.post(u,json=b,timeout=25,proxies=proxies,allow_redirects=False)",
  "r=_s.post(u,json=b,timeout=25,proxies=proxies,allow_redirects=True)",
  ['test_ipleak']),
 ("the vault's doorbell post takes HTTP_PROXY from the environment", 'gs_wake_agent',
  '            urllib.request.ProxyHandler({}), _RefuseRedirect)',
  '            _RefuseRedirect)',
  ['test_ipleak']),
 ('the private scratch directory is listable', 'airgap_tx_signer',
  '    d = tempfile.mkdtemp(dir=base)\n',
  '    d = tempfile.mkdtemp(dir=base)\n    os.chmod(d, 0o755)\n',
  ['test_shmwipe']),
 ('the private scratch directory is named for the toolchain', 'airgap_tx_signer',
  '    d = tempfile.mkdtemp(dir=base)\n',
  '    d = tempfile.mkdtemp(prefix="gs_scratch_", dir=base)\n',
  ['test_shmwipe']),
 ('the private scratch directory outlives the process', 'airgap_tx_signer',
  '    atexit.register(_drop_private_scratch, d)\n',
  '',
  ['test_shmwipe']),
 ('the output import stages in the shared root', 'airgap_tx_signer',
  '        _iw = tempfile.mkdtemp(prefix="gs_impout_", dir=_pw_scratch_dir())\n',
  '        _iw = tempfile.mkdtemp(prefix="gs_impout_", dir="/dev/shm" if os.path.isdir("/dev/shm") else None)\n',
  ['test_shmwipe']),
 ("the wipe does not look inside a killed signer's private scratch", 'paranoia_mode',
  '            if not (entry.name.startswith(GS_TEMP_PREFIXES)\n                    or _is_private_scratch(entry, uid)):\n',
  '            if not entry.name.startswith(GS_TEMP_PREFIXES):\n',
  ['test_shmwipe']),
 ("the wipe takes a temp directory holding one of our names among others", 'paranoia_mode',
  '    return bool(names) and all(n.startswith(GS_TEMP_PREFIXES) for n in names)\n',
  '    return bool(names) and any(n.startswith(GS_TEMP_PREFIXES) for n in names)\n',
  ['test_shmwipe']),
 ('the gpg recipient is read from argv only', 'thor_swap_preparer',
  '    args.gpg_recipient = env_or_argv("GS_GPG_RECIPIENT", args.gpg_recipient,\n                                     "The GPG recipient")\n',
  '    pass\n',
  ['test_swap_receive']),
 ('the gpg bundle names its key', 'thor_swap_preparer',
  '                                      extra_args=["--throw-keyids"])\n',
  '                                      )\n',
  ['test_swap_receive']),
 ("the wipe does not reach the toolchain's own __pycache__", 'paranoia_mode',
  '    return list(PYCACHE_GLOBS) + [str(Path(_gsm.TOOLCHAIN_DIR)\n                                      / "**/__pycache__")]\n',
  '    return list(PYCACHE_GLOBS)\n',
  ['test_opsec_guarantees']),
 ('the mixer writes bytecode beside itself', 'GhostSpiral',
  'sys.dont_write_bytecode = True   # no __pycache__: its mtimes date the first run\n',
  '',
  ['test_opsec_guarantees']),
 ('the wake agent writes bytecode beside itself', 'gs_wake_agent',
  'sys.dont_write_bytecode = True   # no __pycache__: its mtimes date the first run\n',
  '',
  ['test_opsec_guarantees']),
 ("the issued mark's id is a plain digest of address 0", 'gs_btc_watch.py',
  '    return hmac.new(hd.chain_code, b"gs-issued-mark-v2:" + hd.key.sec(),\n                    hashlib.sha256).hexdigest()[:16]\n',
  '    return hashlib.sha256(("addr0:" + str(derive_receive_address(xpub, 0, network))).encode()).hexdigest()[:16]\n',
  ['test_wake_agent']),
 ('a mark under the old name is not read', 'gs_wake_agent',
  '    _old = _legacy_mark_path(key, p)\n    best, _from = None, None\n',
  '    _old = None\n    best, _from = None, None\n',
  ['test_wake_agent']),
 ('a mark carrying the old id is not this chain\'s', 'gs_wake_agent',
  '    _ids = (_xpub_id(key), _legacy_xpub_id(key))\n',
  '    _ids = (_xpub_id(key),)\n',
  ['test_wake_agent']),
 ('of two marks for one chain the first read stands', 'gs_wake_agent',
  '        if best is None or _n > best["issued"]:\n',
  '        if best is None:\n',
  ['test_wake_agent']),
 ('a mark found under the old id is left there', 'gs_wake_agent',
  '        _migrate_issued_mark(key, p, _old, best["issued"])\n',
  '        pass\n',
  ['test_wake_agent']),
 ('a write leaves a mark under the old name', 'gs_wake_agent',
  '    if _old is not None and (_old.exists() or _old.is_symlink()):\n',
  '    if False:\n',
  ['test_wake_agent']),
 ("the mark's times say the second it was written", 'gs_wake_agent',
  '_once("wake", "btc_issued_mark_legacy_left")\n    _blur_mark_times(p)\n',
  '_once("wake", "btc_issued_mark_legacy_left")\n',
  ['test_wake_agent']),
 ("the marks' directory keeps the second it changed", 'gs_wake_agent',
  '    for _q in (p, p.parent):\n',
  '    for _q in (p,):\n',
  ['test_wake_agent']),
 ('reading the mark moves its atime', 'gs_wake_agent',
  '        fd = os.open(str(q), os.O_RDONLY | getattr(os, "O_NOATIME", 0))\n',
  '        fd = os.open(str(q), os.O_RDONLY)\n',
  ['test_wake_agent']),
 ('pairing does not see a mark under the old id', 'gs_wake_keys',
 ...
 ("the hex reaches the server as the caller cased it", 'gs_btc_broadcast.py',
  ...
  '    return h\n',
  ['test_btc_broadcast']),
 # ---- the stage 2 read: what one party's word still decided ----
 ('a pool ThorChain does not swap through is quoted against', 'btc_forwarder',
  '        if str(pool.get("status") or "").lower() != "available":\n',
  '        if False:\n',
  ['test_btc_forwarder']),
 ('the pools are read and not measured against', 'btc_forwarder',
  '                         (pool_rate, "ThorChain pool price")):\n',
  '                         (None, "ThorChain pool price")):\n',
  ['test_btc_forwarder']),
 ('the pools are never read', 'btc_forwarder',
  '            pool_rate = thornode_pool_rate(args.thornode, args.tor_proxy)\n',
  '            pool_rate = None\n',
  ['test_btc_forwarder']),
 ('a sending run with no price reference quotes and signs', 'btc_forwarder',
  '    if (args.broadcast or args.reconcile) and oracle is None \\\n            and pool_rate is None:\n',
  '    if False:\n',
  ['test_btc_forwarder']),
 ('no price reference is not `delayed`', 'btc_forwarder',
  '        integrity_log("forward", "no_price_reference")\n        write_status(args.outfile, STATUS_DELAYED)\n',
  '        integrity_log("forward", "no_price_reference")\n',
  ['test_btc_forwarder']),
 ('a pool that will not swap is not `delayed`', 'btc_forwarder',
  '    except Refused:\n        write_status(args.outfile, STATUS_DELAYED)\n        raise\n    if (args.broadcast or args.reconcile) and oracle is None',
  '    except Refused:\n        raise\n    if (args.broadcast or args.reconcile) and oracle is None',
  ['test_btc_forwarder']),
 ('a memo with a space or a non-ASCII character is signed as it came', 'btc_forwarder',
  '    if any(not 0x21 <= ord(ch) <= 0x7E for ch in text):\n',
  '    if False:\n',
  ['test_btc_forwarder']),
 ('a memo naming an asset ThorChain does not have is signed', 'btc_forwarder',
  '    if text.split(":")[1].upper() not in ("XMR", "XMR.XMR"):\n',
  '    if False:\n',
  ['test_btc_forwarder']),
 ("the locktime is one server's tip", 'btc_forwarder',
  '        if _th is not None and _th < lock_tip:\n',
  '        if False:\n',
  ['test_btc_forwarder']),
 ('the capped locktime is computed and not used', 'btc_forwarder',
  '            chosen, lock_tip, inbound_spk, send_sat, fee_sat, memo_b,\n',
  '            chosen, picture["tip"], inbound_spk, send_sat, fee_sat, memo_b,\n',
  ['test_btc_forwarder']),
 ("ThorChain's BTC height of 0 is taken, and nLockTime is 0", 'btc_forwarder',
  '    # less would -- to nLockTime 0 -- and is no height.\n    if isinstance(h, bool) or not isinstance(h, int) or h <= 0:\n',
  '    # less would -- to nLockTime 0 -- and is no height.\n    if isinstance(h, bool) or not isinstance(h, int):\n',
  ['test_btc_forwarder']),
 ("one server's fee estimate is paid whatever ThorChain says", 'btc_forwarder',
  '            and not isinstance(est, bool) and est > cap:\n',
  '            and not isinstance(est, bool) and False:\n',
  ['test_btc_forwarder']),
 ("the operator's own fee rate is capped too", 'btc_forwarder',
  '    if args.feerate_sat_vb:\n        return args.feerate_sat_vb\n    est = picture.get("fee_sat_vb")\n',
  '    est = args.feerate_sat_vb or picture.get("fee_sat_vb")\n',
  ['test_btc_forwarder']),
 ('a gas rate in units this tool does not read caps the fee', 'btc_forwarder',
  '    if units not in (None, "satsperbyte"):\n        return None\n',
  '',
  ['test_btc_forwarder']),
 ("the fee cap is three times ThorChain's rate", 'btc_forwarder',
  'GAS_RATE_CAP_X = 2\n',
  'GAS_RATE_CAP_X = 3\n',
  ['test_btc_forwarder']),
 ("THORNode's inbound list is asked twice a run", 'btc_forwarder',
  '            entries=_thor_entries_or_refuse(args))\n',
  '            entries=None)\n',
  ['test_btc_forwarder']),
 ("the vault's slippage stop never reaches the forwarder", 'gs_wake_agent',
  '                "--max-slippage", str(Decimal(_slip) / Decimal(10000)),\n',
  '                "--max-slippage", "0.25",\n',
  ['test_wake_agent']),
 ('a slippage stop on the keyfile is coerced', 'gs_wake_agent',
  '        _slip = _btc_setting(key, "btc_max_slippage_bps", 2500, 100, 5000)\n',
  '        _slip = int(key.get("btc_max_slippage_bps") or 2500)\n',
  ['test_wake_agent']),
 ('pairing takes any slippage stop', 'gs_wake_keys',
  '    if not 100 <= int(getattr(args, "max_slippage_bps", 2500)) <= 5000:\n',
  '    if False:\n',
  ['test_wake_agent']),
 # ---- the open items: an undelivered deposit, a retired bundle ----
 ('the vault refuses a deposit note that names its undelivered predecessor', 'gs_wake_proto.py',
  '    if not (want <= got <= want | opt):\n        missing = sorted(want - got)\n',
  '    if got != want:\n        missing = sorted(want - got)\n',
  ['test_wake_protocol', 'test_multi_client']),
 ('an optional field is accepted and then dropped', 'gs_wake_proto.py',
  '        if k not in body:\n            continue\n        try:\n            params[k] = check(body[k])\n        except WakeError as e:\n',
  '        if True:\n            continue\n        try:\n            params[k] = check(body[k])\n        except WakeError as e:\n',
  ['test_wake_protocol', 'test_multi_client']),
 ('a hand-poked deposit may not name its undelivered predecessor', 'gs_doorbell',
  '    if not (want <= got <= want | opt):\n        raise Doorbell(\n',
  '    if got != want:\n        raise Doorbell(\n',
  ['test_wake_doorbell']),
 ("the vault never releases an undelivered deposit's place", 'gs_wake_agent',
  '            _rel = _release_undelivered(artifact_dir, params["replaces"],\n',
  '            _rel = None and _release_undelivered(artifact_dir, params["replaces"],\n',
  ['test_multi_client']),
 ("any owner may release a deposit's place", 'gs_wake_agent',
  '                or rec.get("owner") != owner or rec.get("spent") is True:\n',
  '                or rec.get("spent") is True:\n',
  ['test_multi_client']),
 ('a released deposit keeps its youth grace all the same', 'gs_wake_agent',
  '            and not rec.get("released"):\n        return True\n',
  '            and True:\n        return True\n',
  ['test_multi_client']),
 ('the pager never names the undelivered deposit', 'gs_telegram_pager',
  '                    params["replaces"] = _prev\n',
  '                    pass\n',
  ['test_telegram_pager']),
 ('a deposit that arrived does not answer for the undelivered one', 'gs_telegram_pager',
  '                self._undelivered().pop(chat_id, None)\n',
  '                pass\n',
  ['test_telegram_pager']),
 ('an undelivered pay message is not remembered', 'gs_telegram_pager',
  '                self._btc_forget(h)\n                self._mark_undelivered(chat_id, h, job)\n',
  '                self._btc_forget(h)\n',
  ['test_telegram_pager']),
 ('an undelivered note is not remembered', 'gs_telegram_pager',
  '                    _LAST_MID.failed = True\n                    self._mark_undelivered(chat_id, h, job)\n',
  '                    _LAST_MID.failed = True\n',
  ['test_telegram_pager']),
 ('an undelivered sealed slip is not remembered', 'gs_telegram_pager',
  '                _LAST_MID.failed = True\n                self._mark_undelivered(chat_id, h, job)\n                # NOT "/watch {h}".',
  '                _LAST_MID.failed = True\n                # NOT "/watch {h}".',
  ['test_telegram_pager']),
 ("an undelivered deposit is named past the vault's own grace", 'gs_telegram_pager',
  '        if time.time() - float(_u[1]) >= proto.DEPOSIT_PLACE_TTL_S:\n',
  '        if False:\n',
  ['test_telegram_pager']),
 ('any job is remembered as an undelivered deposit', 'gs_telegram_pager',
  '        if job == "receive_and_quote" and isinstance(h, str) \\\n',
  '        if isinstance(h, str) \\\n',
  ['test_telegram_pager']),
 ("a retired record's shredded bundle is never rebuilt", 'gs_wake_agent',
  '        if rec.get("spent") is True and not Path(str(bundle)).is_file():\n',
  '        if False:\n',
  ['test_wake_agent']),
 ('a bundle is rebuilt whatever the wallet says', 'gs_wake_agent',
  '    if held != pair:\n',
  '    if False:\n',
  ['test_wake_agent']),
 ('plans naming two destinations rebuild from one of them', 'gs_wake_agent',
  '    if pair is None or len(dests) != 1 or not inside:\n',
  '    if pair is None or not dests or not inside:\n',
  ['test_wake_agent']),
 ('a rebuilt bundle is written readable by all', 'gs_wake_agent',
  '                      target, perms=0o600)\n    integrity_log("wake", "bundle_rebuilt")\n',
  '                      target, perms=0o644)\n    integrity_log("wake", "bundle_rebuilt")\n',
  ['test_wake_agent']),
 # ---- the review of stage 2: one rate a run, a ThorChain read that is a fact ----
 ('the bump reads the raw estimate the fee step never pays', 'btc_forwarder',
  '    _est = todays_rate(args, picture)\n',
  '    _est = picture.get("fee_sat_vb")\n',
  ['test_btc_forwarder']),
 ("ThorChain's bound is not held to the operator's ceiling", 'btc_forwarder',
  '    if rate <= int(ceiling):\n        cap = min(cap, int(ceiling))\n',
  '    if False:\n        cap = min(cap, int(ceiling))\n',
  ['test_btc_forwarder']),
 ("a THORNode outage at the fee step is a refusal that takes the fee step's word", 'btc_forwarder',
  '            args._thor_entries = _fetch_inbound(args.thornode,\n',
  '            args._thor_entries = fetch_inbound_entries(args.thornode,\n',
  ['test_btc_forwarder']),
 ('the cross-check asks THORNode again after the fee step could not read it', 'btc_forwarder',
  '            entries=_thor_entries_or_refuse(args))\n',
  '            entries=_thor_entries_once(args))\n',
  ['test_btc_forwarder']),
 ('the stop is named rounded to a whole percent', 'btc_forwarder',
  '            _stop = format((Decimal(str(max_slippage)) * 100).normalize(),\n',
  '            _stop = format((Decimal(str(max_slippage)) * 100).quantize(1),\n',
  ['test_btc_forwarder']),
 # ---- the review of stage 3: a stop is a stop, a liar is no witness twice ----
 ('a submit the stop cut short does not say so', 'gs_btc_broadcast.py',
  '            stopped = True\n            break\n',
  '            break\n',
  ['test_btc_broadcast']),
 ("a stop that cut the send short is read as the list's verdict", 'btc_forwarder',
  '        if sub.get("stopped") and outcome not in bcast.MAYBE_MOVED:\n',
  '        if False:\n',
  ['test_btc_forwarder']),
 ('a stopped send whose bytes left is reported as a stop, the plan unwritten', 'btc_forwarder',
  '        if sub.get("stopped") and outcome not in bcast.MAYBE_MOVED:\n',
  '        if sub.get("stopped"):\n',
  ['test_btc_forwarder']),
 ("a stopped re-send writes the list's verdict over `accepted`", 'btc_forwarder',
  '    outcome = sub["outcome"]\n    if sub.get("stopped") and outcome not in bcast.MAYBE_MOVED:\n',
  '    outcome = sub["outcome"]\n    if False:\n',
  ['test_btc_forwarder']),
 ('seen asks a caught-out server when it is the only one', 'gs_btc_broadcast.py',
  '    order = [o for o in order if o[0] not in _distrust]\n',
  '',
  ['test_btc_broadcast']),
 ('the acceptor gives way to servers that are all caught out', 'gs_btc_broadcast.py',
  '    order = [o for o in order if o[0] not in _distrust]\n    if _avoid and len(order) > 1:\n        order = [o for o in order if o[0] not in _avoid]\n',
  '    if _avoid and len(order) > 1:\n        order = [o for o in order if o[0] not in _avoid]\n    order = [o for o in order if o[0] not in _distrust]\n',
  ['test_btc_broadcast']),
 ('the plan forgets which server was caught out', 'btc_forwarder',
  '        "broadcast_distrusted": _distrusted(sub, plan0 if why else None),\n',
  '        "broadcast_distrusted": [],\n',
  ['test_btc_forwarder']),
 ("a re-send's seen forgets the plan's caught-out servers", 'btc_forwarder',
  '                            distrust=plan["broadcast_distrusted"])\n',
  '                            distrust=_distrusted(sub))\n',
  ['test_btc_forwarder']),
 ("the reconciliation takes any server's listing as proof", 'btc_forwarder',
  '    spends, _witness = _witnessed(args, plan, spends, address, servers)\n',
  '    _witness = ""\n',
  ['test_btc_forwarder']),
 ("the acceptor's own listing is proof with another server to ask", 'btc_forwarder',
  '    if by not in distrust and not (by in _accs and len(_hosts) > 1):\n',
  '    if by not in distrust:\n',
  ['test_btc_forwarder']),
 ("a trusted server's confirmation is not taken", 'btc_forwarder',
  '    if sr["seen"]:\n        hit["server"], hit["height"] = sr["server"], sr["height"]\n',
  '    if False:\n        hit["server"], hit["height"] = sr["server"], sr["height"]\n',
  ['test_btc_forwarder']),
 ('an unwitnessed spend reads as a server contradicting itself', 'btc_forwarder',
  '    if _witness == "unwitnessed":\n',
  '    if False:\n',
  ['test_btc_forwarder']),
 # ---- the review of stage 4: a paid place is timed, an unwatched deposit is refused first ----
 ("a paid intake deposit's place is held for ever", 'gs_wake_agent',
  '    if _paid is not None and int(now) - _paid < proto.DEPOSIT_PLACE_TTL_S:\n',
  '    if _paid is not None:\n',
  ['test_multi_client']),
 ("the money stamp is not read, only the first forward's bucket", 'gs_wake_agent',
  '    _ts = [v for v in (rec.get("btc_money_at"), rec.get("forwarded"))\n',
  '    _ts = [v for v in (rec.get("forwarded"),)\n',
  ['test_multi_client']),
 ('a money stamp off the intake holds a place', 'gs_wake_agent',
  '    if rec.get("btc_index") is None:\n        return None\n    _ts = [',
  '    if False:\n        return None\n    _ts = [',
  ['test_multi_client']),
 ("a `delayed` or `returned` answer stamps nothing", 'gs_wake_agent',
  '                if _fw == "delayed" and handle in handles:\n',
  '                if False:\n',
  ['test_wake_agent']),
 ("a stranger's `seen` or `short` holds the intake's place", 'gs_wake_agent',
  '                if _fw == "delayed" and handle in handles:\n',
  '                if handle in handles:\n',
  ['test_wake_agent']),
 ('a sending forward stamps nothing', 'gs_wake_agent',
  '        if handles[handle]["forward_sent"] \\\n',
  '        if False \\\n',
  ['test_wake_agent']),
 ('the vault issues a deposit to an end that cannot watch it', 'gs_wake_agent',
  '        if params.get("unwatched") is True and btc_mode(key):\n',
  '        if False:\n',
  ['test_multi_client']),
 ('the vault refuses an unwatched deposit off the intake too', 'gs_wake_agent',
  '        if params.get("unwatched") is True and btc_mode(key):\n',
  '        if params.get("unwatched") is True:\n',
  ['test_multi_client']),
 ('the unwatched refusal carries no word', 'gs_wake_agent',
  '_REFUSAL_PHASE = {"at_capacity": "full", "intake_unwatched": "unwatched"}',
  '_REFUSAL_PHASE = {"at_capacity": "full"}',
  ['test_multi_client']),
 ('the pager never says it cannot watch', 'gs_telegram_pager',
  '                if not self.btc_servers:\n                    params["unwatched"] = True\n',
  '                if False:\n                    params["unwatched"] = True\n',
  ['test_telegram_pager']),
 ("the vault's unwatched refusal reads as the generic line", 'gs_telegram_pager',
  '            elif _rphase == "unwatched" and job == "receive_and_quote":\n',
  '            elif False:\n',
  ['test_multi_client']),
 ('the Pi says "try again later" about a deposit it will not show', 'gs_telegram_pager',
  '                    self.send(chat_id, UNWATCHED_ANSWER,\n                              buttons=MENU_BUTTONS)\n',
  '                    self.send(chat_id, "Try again later.",\n                              buttons=MENU_BUTTONS)\n',
  ['test_telegram_pager']),
 ('the unwatched flag is coerced from any truthy value', 'gs_wake_proto.py',
  '    if v is not True:\n        raise WakeError("expected true")\n',
  '    if not v:\n        raise WakeError("expected true")\n',
  ['test_wake_protocol']),
 ('a deposit note may not say it is unwatched', 'gs_wake_proto.py',
  '"optional": {"replaces": _handle_field, "unwatched": _true_field},',
  '"optional": {"replaces": _handle_field},',
  ['test_wake_protocol', 'test_multi_client']),
 # ---- the review of stage 5: the kept mark, the keep list, the record ----
 ('the kept mark leaves out the dust a sweep takes', 'btc_forwarder',
  '        _back = _largest_first([unspent[o] for o in _kept_ex\n                                if o in unspent])\n',
  '        _back = list(outputs)\n',
  ['test_btc_forwarder']),
 ('a leftover flood rides on the history window unbounded', 'btc_forwarder',
  '    _keep |= set(_extra[:KEEP_EXTRA_MAX])\n',
  '    _keep |= set(_extra)\n',
  ['test_btc_forwarder']),
 ('the kept funding gives way to the leftover flood', 'btc_forwarder',
  '    for _c0, t in _cands:\n',
  '    for _c0, t in [c for c in _cands if c[0][2] == 1]:\n',
  ['test_btc_forwarder']),
 ("only the current plan's mark names a record", 'gs_wake_agent',
  '            if _r and _r not in _named:\n',
  '            if _r and _r not in _named and _pos == 0:\n',
  ['test_wake_agent']),
 ("a rotated plan's stale mark outranks the current plan's", 'gs_wake_agent',
  '                    -_named[_t][0] if _t in _named else 0,\n',
  '                    0,\n',
  ['test_wake_agent']),
 ('a rotated mark stands in for a record the chain does not hold', 'gs_wake_agent',
  '            if (_pos != 0 and _r not in _held) \\\n',
  '            if False \\\n',
  ['test_wake_agent']),
 ("the acceptor's lone listing is dropped while the other server is down", 'btc_forwarder',
  '    if by not in distrust:\n        integrity_log("forward", "listed_unconfirmed")\n',
  '    if False:\n        integrity_log("forward", "listed_unconfirmed")\n',
  ['test_btc_forwarder']),
 ("an unconfirmed listing is taken as proof, the kept bytes dropped", 'btc_forwarder',
  '        if _witness != "unconfirmed":\n',
  '        if True:\n',
  ['test_btc_forwarder']),
 # ---- the residuals: our own inputs, the black-hole acceptor, `delayed` ----
 ("our forward's inputs are named only from their funding's fetch", 'btc_forwarder',
  '    _own_inputs(spends, [plan] + _chain0, _sent_here)\n',
  '',
  ['test_btc_forwarder']),
 ("any bytes name our forward's inputs, whatever txid they hash to", 'btc_forwarder',
  '        if _tx.txid().hex() != str(s.get("txid") or "").lower():\n            continue\n',
  '',
  ['test_btc_forwarder']),
 ('submit tries the suspects in rotation like any other', 'gs_btc_broadcast.py',
  '    order = ([o for o in order if o[0] not in _last]\n             + [o for o in order if o[0] in _last])\n',
  '',
  ['test_btc_broadcast']),
 ('a re-send hands the black-hole acceptor the bytes first', 'btc_forwarder',
  '                           stop=shutdown_requested, last=_suspects(plan))\n',
  '                           stop=shutdown_requested)\n',
  ['test_btc_forwarder']),
 ("a reconciliation's fresh forward tries the suspects first", 'btc_forwarder',
  '                               last=_suspects(plan0) if why else None)\n',
  '                               last=None)\n',
  ['test_btc_forwarder']),
 ('an unwitnessed acceptance is not recorded', 'btc_forwarder',
  '        if seen_r and seen_r.get("seen"):\n            out.discard(_acc)\n        else:\n            out.add(_acc)\n',
  '        if seen_r and seen_r.get("seen"):\n            out.discard(_acc)\n',
  ['test_btc_forwarder']),
 ('a witnessed acceptor is tried last all the same', 'btc_forwarder',
  '        if seen_r and seen_r.get("seen"):\n            out.discard(_acc)\n        else:\n            out.add(_acc)\n',
  '        out.add(_acc)\n',
  ['test_btc_forwarder']),
 ("a plan from before the field names no suspect", 'btc_forwarder',
  '    if "broadcast_unwitnessed" not in plan and plan.get("seen") is not True \\\n',
  '    if False \\\n',
  ['test_btc_forwarder']),
 ('a contradicted listing is not re-sent while the look reads its inputs spent', 'btc_forwarder',
  '                        or _witness == "contradicted"):\n',
  '                        or False):\n',
  ['test_btc_forwarder']),
 ('a listing a trusted server contradicts reads as nobody to ask', 'btc_forwarder',
  '            "contradicted" if sr["asked"] else "unwitnessed")\n',
  '            "unwitnessed")\n',
  ['test_btc_forwarder']),
 ('`delayed` does not mark the record', 'gs_wake_agent',
  '                    if _fw == "delayed":\n                        handles[handle]["btc_delayed"] = \\\n',
  '                    if False:\n                        handles[handle]["btc_delayed"] = \\\n',
  ['test_wake_agent']),
 ('another answer leaves the `delayed` mark standing', 'gs_wake_agent',
  '                    else:\n                        handles[handle].pop("btc_delayed", None)\n',
  '                    else:\n                        pass\n',
  ['test_wake_agent']),
 ("a reconciliation's own `delayed` is cleared by the plan it wrote", 'gs_wake_agent',
  '        if _fw_run != "delayed":\n            handles[handle].pop("btc_delayed", None)\n',
  '        if True:\n            handles[handle].pop("btc_delayed", None)\n',
  ['test_wake_agent']),
 ('a forward that sent leaves the `delayed` mark standing', 'gs_wake_agent',
  '        if _fw_run != "delayed":\n            handles[handle].pop("btc_delayed", None)\n',
  '        if False:\n            handles[handle].pop("btc_delayed", None)\n',
  ['test_wake_agent']),
 ("a `delayed` deposit's place goes with the clock", 'gs_wake_agent',
  '            and isinstance(rec.get("btc_delayed"), int) \\\n',
  '            and False \\\n',
  ['test_multi_client']),
 # ---- the review of 657deae: V1-V5, P1, C1-C8, the doorbell ----
 ('a minting job runs past an unsettled mix mark', 'gs_wake_agent',
  '    if job in _MINTING_JOBS and _pending_marks(led):\n',
  '    if False:\n',
  ['test_wake_agent', 'test_multi_client']),
 ('a withdrawal is not a minting job', 'gs_wake_agent',
  '_MINTING_JOBS = ("receive_and_quote", "withdraw")\n',
  '_MINTING_JOBS = ("receive_and_quote",)\n',
  ['test_wake_agent', 'test_multi_client']),
 ('a deposit is not a minting job', 'gs_wake_agent',
  '_MINTING_JOBS = ("receive_and_quote", "withdraw")\n',
  '_MINTING_JOBS = ("withdraw",)\n',
  ['test_wake_agent', 'test_multi_client']),
 ("a mix mark is written over another owner's", 'gs_wake_agent',
  '    if _pending_marks(led):\n        integrity_log("wake", "mix_unsettled")\n',
  '    if False:\n        integrity_log("wake", "mix_unsettled")\n',
  ['test_wake_agent']),
 ('a re-minted account another owner holds is handed to the next deposit', 'gs_wake_agent',
  '                if _np is not None and (_held_by or any(\n',
  '                if _np is not None and (False or any(\n',
  ['test_wake_agent']),
 ("an account another owner holds is recorded as this owner's too", 'gs_wake_agent',
  '            if _account_owners(led, _i) - {owner}:\n',
  '            if False:\n',
  ['test_wake_agent']),
 ('a withdrawal mixes on a wallet whose account list went backwards', 'gs_wake_agent',
  '        if _owner and _owned_all - set(_accts_before):\n',
  '        if False:\n',
  ['test_wake_agent']),
 ('the idle-boot fee sweep switch is coerced', 'gs_wake_agent',
  '            "on_idle_boot": _key_switch(key, "fee_sweep_on_idle_boot",\n                                        "fee_sweep_misconfigured")}\n',
  '            "on_idle_boot": bool(key.get("fee_sweep_on_idle_boot"))}\n',
  ['test_wake_agent']),
 ('fee_sweep_depth true reads as depth 1', 'gs_wake_agent',
  '    if isinstance(depth, bool) or depth not in proto.WITHDRAW_DEPTHS:\n',
  '    if depth not in proto.WITHDRAW_DEPTHS:\n',
  ['test_wake_agent']),
 ("a retired record's path outside the artifact dir is shredded", 'gs_wake_agent',
  '            if Path(str(_p)).resolve().parent != _root:\n',
  '            if False:\n',
  ['test_wake_agent']),
 ('the locked figure sums arrivals the next leg cannot take as one', 'gs_wake_agent',
  '                if _bal > _unl and _bal > _best:\n                    _best = _bal\n',
  '                if _bal > _unl:\n                    _best += _bal\n',
  ['test_wake_agent', 'test_multi_client']),
 ('a configuration kind is written per deposit', 'gs_telegram_pager',
  '            if kind in _said:\n                return\n',
  '',
  ['test_telegram_pager']),
 ('an unconfirmed listing is not pushed to the other servers', 'btc_forwarder',
  '            _push_elsewhere(args, plan, address, servers, args._push_skip)\n',
  '            pass\n',
  ['test_btc_forwarder']),
 ('the push hands the bytes back to the acceptor', 'btc_forwarder',
  '              and x[0] not in set(skip or ())]\n',
  '              ]\n',
  ['test_btc_forwarder']),
 ('an unconfirmed listing is pushed with nobody asked', 'btc_forwarder',
  '        if sr["asked"]:\n            args._push_skip = sorted(_accs | {by})\n',
  '        if True:\n            args._push_skip = sorted(_accs | {by})\n',
  ['test_btc_forwarder']),
 ('two ports of one host count as two witnesses', 'btc_forwarder',
  '    _hosts = {x[0] for x in servers if isinstance(x, (tuple, list)) and x}\n',
  '    _hosts = list(servers)\n',
  ['test_btc_forwarder']),
 ('a tap on a forward mined long ago restamps the place', 'gs_wake_agent',
  '                and not (_mined_now and _mined_before):\n',
  '                and True:\n',
  ['test_wake_agent']),
 ('the run that first finds the forward mined stamps nothing', 'gs_wake_agent',
  '                and not (_mined_now and _mined_before):\n',
  '                and not _mined_now:\n',
  ['test_wake_agent']),
 ('a forward of returned money after a mined one stamps nothing', 'gs_wake_agent',
  '                and not (_mined_now and _mined_before):\n',
  '                and not _mined_before:\n',
  ['test_wake_agent']),
 ("`returned` (a stranger's dust says it) holds the place", 'gs_wake_agent',
  '                if _fw == "delayed" and handle in handles:\n',
  '                if _fw in ("delayed", "returned") and handle in handles:\n',
  ['test_wake_agent']),
 ('the plan before the run is never read as mined', 'gs_wake_agent',
  '    _mined_before = (job == "forward_to_swap" and bool(handle)\n                     and _forward_mined(artifact_dir, handle))\n',
  '    _mined_before = False\n',
  ['test_wake_agent']),
 ('an intake deposit is released without asking its BTC address', 'gs_wake_agent',
  '        if rec.get("btc_index") is not None:\n            _stop_if_asked()\n',
  '        if False:\n            _stop_if_asked()\n',
  ['test_wake_agent', 'test_multi_client']),
 ('the release is not handed the paired key', 'gs_wake_agent',
  '        if _release_undelivered(artifact_dir, _btc_release,\n                                params.get("owner"), key=key,\n',
  '        if _release_undelivered(artifact_dir, _btc_release,\n                                params.get("owner"), key=None,\n',
  ['test_multi_client']),
 ('the release ignores the injected look', 'gs_wake_agent',
  '                                unused=d.get("btc_unused")) is not True \\\n',
  '                                unused=None) is not True \\\n',
  ['test_multi_client']),
 ('a record of another chain is asked about on this one', 'gs_wake_agent',
  '                    and _on_this_chain(rec, key):\n                try:\n                    _addr = _btc_address(key, _idx)\n',
  '                    and True:\n                try:\n                    _addr = _btc_address(key, _idx)\n',
  ['test_wake_agent']),
 ('a hand move must spend kept outputs only (late dust cries seed leak)', 'btc_forwarder',
  '        _hand = bool(_kept_ops and _all is not None and _all & _kept_ops)\n',
  '        _hand = bool(_kept_ops and _all is not None and _all <= _kept_ops)\n',
  ['test_btc_forwarder']),
 ('dust beside kept money needs no kept money', 'btc_forwarder',
  '        _hand = bool(_kept_ops and _all is not None and _all & _kept_ops)\n',
  '        _hand = bool(_kept_ops and _all is not None)\n',
  ['test_btc_forwarder']),
 ('an outpoint a plan of ours signed passes as dust', 'btc_forwarder',
  '            _over = bool(_extra_ins & _signed_ops or _signed_hit or _unknown\n',
  '            _over = bool(_signed_hit or _unknown\n',
  ['test_btc_forwarder']),
 ('the dust allowance excludes its own bound', 'btc_forwarder',
  '                         > dust_line(int(args.feerate_ceiling)))\n',
  '                         >= dust_line(int(args.feerate_ceiling)))\n',
  ['test_btc_forwarder']),
 ("the dust allowance is the floor's line", 'btc_forwarder',
  '                         > dust_line(int(args.feerate_ceiling)))\n',
  '                         > dust_line(int(args.feerate_floor)))\n',
  ['test_btc_forwarder']),
 ("the dust allowance ignores the pair's ceiling", 'btc_forwarder',
  '                         > dust_line(int(args.feerate_ceiling)))\n',
  '                         > dust_line(200))\n',
  ['test_btc_forwarder']),
 ('an extra of no stated value counts as nothing', 'btc_forwarder',
  '                                 and not isinstance(_v, bool) and _v > 0\n                                 else None)\n',
  '                                 and not isinstance(_v, bool) and _v > 0\n                                 else 0)\n',
  ['test_btc_forwarder']),
 ("the keep list's extras are re-sorted by txid", 'btc_forwarder',
  '    _cands.sort(key=lambda c: c[0])\n',
  '    _cands.sort(key=lambda c: c[1])\n',
  ['test_btc_forwarder']),
 ('the kept mark is written in txid order', 'btc_forwarder',
  '        _back = _largest_first([unspent[o] for o in _kept_ex\n                                if o in unspent])\n',
  "        _back = sorted([unspent[o] for o in _kept_ex\n                                if o in unspent], key=lambda u: (str(u['tx_hash']).lower(), int(u['vout'])))\n",
  ['test_btc_forwarder']),
 ('what was moved by hand comes after the kept dust', 'btc_forwarder',
  '                (0, _pair_list(_p0.get("returned_moved"))),\n                (0, _pair_list((_rk0 or {}).get("outpoints")\n                               if isinstance(_rk0, dict) else None)),\n',
  '                (0, _pair_list((_rk0 or {}).get("outpoints")\n                               if isinstance(_rk0, dict) else None)),\n                (0, _pair_list(_p0.get("returned_moved"))),\n',
  ['test_btc_forwarder']),
 ('a partial hand move re-sorts the mark', 'btc_forwarder',
  '                _left = [o for o in _pair_list(_rk.get("outpoints"))\n                         if o not in _ins_all]\n',
  '                _left = sorted(o for o in _pair_list(_rk.get("outpoints"))\n                         if o not in _ins_all)\n',
  ['test_btc_forwarder']),
 ('the leftover mark is written in txid order', 'btc_forwarder',
  '                for u in _largest_first(picture["utxos"]):\n',
  '                for u in sorted(picture["utxos"], key=lambda u: (str(u[\'tx_hash\']).lower(), int(u[\'vout\']))):\n',
  ['test_btc_forwarder']),
 ('a hand move is recorded in txid order', 'btc_forwarder',
  '                       for i in _largest_first(_in_dicts)])]\n',
  "                       for i in (lambda _x: sorted(_x, key=lambda u: (str(u.get('tx_hash') or ''), int(u.get('vout') or 0))))(_in_dicts)])]\n",
  ['test_btc_forwarder']),
 ('a corrupt mark crashes the ordered reader', 'btc_forwarder',
  '    out, seen = [], set()\n    if not isinstance(seq, (list, tuple)):\n        return out\n',
  '    out, seen = [], set()\n',
  ['test_btc_forwarder']),
 ('the stand-in is looked up under its own txid', 'gs_wake_agent',
  '            counted.append(dict(_stand, _refund_of=_r))\n',
  '            counted.append(dict(_stand))\n',
  ['test_wake_agent']),
 ("the aggregator is asked for a quote THORNode's word will refuse", 'btc_forwarder',
  '    if args.thornode:\n        thor_btc_entry(_thor_entries_or_refuse(args))\n',
  '',
  ['test_btc_forwarder']),
 ('the by-hand terminal renders only `full`', 'gs_doorbell',
  '        if _rph in getattr(proto, "REFUSAL_PHASES", ()) \\\n',
  '        if _rph == "full" \\\n',
  ['test_wake_doorbell']),
 ('a server the push caught lying stays a witness', 'btc_forwarder',
  '    # opinion could ask it and take its "listed" as proof.\n    plan["broadcast_distrusted"] = _distrusted(sub, plan)\n',
  '    # opinion could ask it and take its "listed" as proof.\n',
  ['test_btc_forwarder']),
 # ---- the three reviews of the fixes: forwarder, vault, Pi ----
 ('a contradicted or stale-look re-send goes out beside our own forward over the same inputs', 'btc_forwarder',
  '    if plan_inputs and _over:\n',
  '    if plan_inputs and _over and not (plan_inputs <= set(unspent)\n                                      or _witness == "contradicted"):\n',
  ['test_btc_forwarder']),
 ('a recorded forward with no plan is not named from its bytes', 'btc_forwarder',
  '        if p is None and _t not in _signed:\n',
  '        if p is None:\n',
  ['test_btc_forwarder']),
 ('the dust allowance is one per spend', 'btc_forwarder',
  '                         or _dust_used + sum(_xval.values())\n',
  '                         or sum(_xval.values())\n',
  ['test_btc_forwarder']),
 ('the dust an earlier move took is not recorded', 'btc_forwarder',
  '                plan["returned_dust"] = [list(o) for o in _pair_list(\n',
  '                _unrecorded = [list(o) for o in _pair_list(\n',
  ['test_btc_forwarder']),
 ('a kept mark is written without its values', 'btc_forwarder',
  '        _note_values(plan, _back)\n',
  '',
  ['test_btc_forwarder']),
 ('a hand move is recorded without its values', 'btc_forwarder',
  '                _note_values(plan, _in_dicts)\n',
  '',
  ['test_btc_forwarder']),
 ('a leftover mark is written without its values', 'btc_forwarder',
  '                    _note_values(plan0, picture["utxos"])\n',
  '',
  ['test_btc_forwarder']),
 ('only the latest acceptor needs a second opinion', 'btc_forwarder',
  '    _accs = set(_unwitnessed_of(plan))\n',
  '    _accs = set()\n',
  ['test_btc_forwarder']),
 ('a server that took the push is not recorded as an acceptor', 'btc_forwarder',
  '        plan["broadcast_unwitnessed"] = sorted(_unwitnessed_of(plan) | {_pa})\n',
  '        pass\n',
  ['test_btc_forwarder']),
 ('the push goes out in a run that replaces the forward', 'btc_forwarder',
  '        if vars(args).get("_push_skip") is not None and stuck is None:\n',
  '        if vars(args).get("_push_skip") is not None:\n',
  ['test_btc_forwarder']),
 ('a pin mismatch during the push is said as a failed push', 'btc_forwarder',
  '        integrity_log("forward", "pin_mismatch")\n        return\n    except Exception:                                        # noqa: BLE001\n        integrity_log("forward", "push_failed")\n',
  '        integrity_log("forward", "push_failed")\n        return\n    except Exception:                                        # noqa: BLE001\n        integrity_log("forward", "push_failed")\n',
  ['test_btc_forwarder']),
 ('the release asks the network before the jitter and the Tor check', 'gs_wake_agent',
  '                                        defer_btc=True)\n',
  '                                        defer_btc=False)\n',
  ['test_multi_client']),
 ('a deferred release the gate needed never refuses', 'gs_wake_agent',
  '        _btc_release_needed = bool(_btc_release and _held) and (\n',
  '        _btc_release_needed = False and (\n',
  ['test_multi_client']),
 ("the stand-in's refund is looked up under its own txid", 'gs_wake_agent',
  '            _b = min(int(_back.get(str(p.get("_refund_of") or p.get("txid")\n',
  '            _b = min(int(_back.get(str(p.get("txid")\n',
  ['test_wake_agent']),
 ('an account two owners hold is spendable by either', 'gs_wake_agent',
  '    return _mine - _theirs\n',
  '    return _mine\n',
  ['test_wake_agent']),
 ('a mark whose wallet shows nothing new is let go at once', 'gs_wake_agent',
  '            if _tries < PENDING_EMPTY_MAX:\n',
  '            if False:\n',
  ['test_wake_agent']),
 ("the empty-read count is dropped by the ledger's load", 'gs_wake_agent',
  '                    owners[k][PENDING_EMPTY_FIELD] = _pe\n',
  '                    pass\n',
  ['test_wake_agent']),
 ('unlocked dust hides a deposit still unlocking', 'gs_wake_agent',
  '            _locked = _locked_value(key, owned_accounts=_owned)\n            if _locked is not None and _locked >= _floor:\n',
  '            _locked = _locked_value(key, owned_accounts=_owned)\n            if False:\n',
  ['test_wake_agent']),
 ('the release looks past a stop', 'gs_wake_agent',
  '        if rec.get("btc_index") is not None:\n            _stop_if_asked()\n',
  '        if rec.get("btc_index") is not None:\n            pass\n',
  ['test_wake_agent']),
 ('a watch and a forward are refused behind a mix mark', 'gs_wake_agent',
  '_MINTING_JOBS = ("receive_and_quote", "withdraw")\n',
  '_MINTING_JOBS = ("receive_and_quote", "withdraw", "watch",\n                 "forward_to_swap")\n',
  ['test_wake_agent']),
 ('a `delayed` record of another BTC account holds its place for good', 'gs_wake_agent',
  '            and (not isinstance(key, dict) or _on_this_chain(rec, key)):\n',
  '            and True:\n',
  ['test_multi_client']),
 ('a wizard burned whole is written on the chain', 'gs_telegram_pager',
  '        if gone != len(mids):\n            self._config_once("wizard_burn_partial")\n',
  '        if True:\n            self._config_once("wizard_burn_partial")\n',
  ['test_telegram_pager']),
 ('a partial wizard burn is written per event', 'gs_telegram_pager',
  '            self._config_once("wizard_burn_partial")\n',
  '            integrity_log("pager", "wizard_burn_partial")\n',
  ['test_telegram_pager']),
 ('a lost reply of a job is not on the chain', 'gs_telegram_pager',
  '            if getattr(_LAST_MID, "failed", False):\n                integrity_log("pager", "reply_undelivered")\n',
  '',
  ['test_telegram_pager']),
 ("a failed send is not noted for the job's worker", 'gs_telegram_pager',
  '            _LAST_MID.failed = True\n            return False\n',
  '            return False\n',
  ['test_telegram_pager']),
 ('a configuration kind known at start waits for the first deposit', 'gs_telegram_pager',
  '    if not _pager.btc_servers:\n        _pager._config_once("intake_unwatched")\n',
  '',
  ['test_telegram_pager']),
 ('the alert line says the bot serves other chats', 'gs_telegram_pager',
  '            self.send(_to, f"{chat_name(job)}: {_w}. It needs someone at "\n                           f"the other end.")\n',
  '            self.send(_to, f"{chat_name(job)}: {_w} for a deposit in "\n                           f"another chat. It needs someone at the other "\n                           f"end.")\n',
  ['test_telegram_pager']),
 ('the by-hand terminal renders any word on a refusal as a reason', 'gs_doorbell',
  '        if _rph in getattr(proto, "REFUSAL_PHASES", ()) \\\n',
  '        if _rph \\\n',
  ['test_wake_doorbell']),
 ('the entry set is not stored before a swap names it', 'GhostSpiral',
  '        persist_entry_set(rpc_primary, receive_mode)\n',
  '',
  ['test_peel_pipeline', 'test_send_gates']),
 ('the wallet is not asked to store before anything is minted', 'GhostSpiral',
  '        persist_before_mint(rpc_primary)\n',
  '',
  ['test_peel_pipeline', 'test_send_gates']),
 ('nothing is stored before the first relay', 'GhostSpiral',
  '        persist_before_relay(rpc_primary, receive_mode)\n',
  '',
  ['test_peel_pipeline']),
 ("the vault's receive-mode runs skip the last store (the review's F3)", 'GhostSpiral',
  '        persist_before_relay(rpc_primary, receive_mode)\n',
  '        if not receive_mode:\n            persist_before_relay(rpc_primary, receive_mode)\n',
  ['test_peel_pipeline']),
 ('the last store comes before the change-sweep accounts are minted', 'GhostSpiral',
  "        change_sweep_jobs = build_change_sweep_jobs(\n            rpc_primary, addr_index, change_accounts)\n        # Where the last rounds pay. The exit gates on these having landed\n        # before it reads the wallet -- see _stage5_run's landing gate.\n        _dag_ix, _sweep_tg = _landing_targets(addr_index, dag_plan,\n                                              change_sweep_jobs)\n        # EVERY ACCOUNT THE ROUNDS PAY INTO IS ON DISK BEFORE THE FIRST ONE\n        # RELAYS. build_change_sweep_jobs, just above, is the last mint of the\n        # run, and _stage5_run's entry veil is its first relay. See\n        # persist_minted.\n        persist_before_relay(rpc_primary, receive_mode)\n",
  "        persist_before_relay(rpc_primary, receive_mode)\n        change_sweep_jobs = build_change_sweep_jobs(\n            rpc_primary, addr_index, change_accounts)\n        # Where the last rounds pay. The exit gates on these having landed\n        # before it reads the wallet -- see _stage5_run's landing gate.\n        _dag_ix, _sweep_tg = _landing_targets(addr_index, dag_plan,\n                                              change_sweep_jobs)\n        # EVERY ACCOUNT THE ROUNDS PAY INTO IS ON DISK BEFORE THE FIRST ONE\n        # RELAYS. build_change_sweep_jobs, just above, is the last mint of the\n        # run, and _stage5_run's entry veil is its first relay. See\n        # persist_minted.\n",
  ['test_peel_pipeline']),
 ('a wallet that will not store is relayed from anyway', 'GhostSpiral',
  '    integrity_log(stage, "wallet_store_FAILED")\n    sys.exit(f"[!] The wallet would not write {what} to its file "\n',
  '    integrity_log(stage, "wallet_store_FAILED")\n    print(f"[!] The wallet would not write {what} to its file "\n',
  ['test_peel_pipeline', 'test_send_gates']),
 ('a send-mode run stops after the swap instead of waiving (the review\'s F1)', 'GhostSpiral',
  '        waive=("" if receive_mode else\n',
  '        waive=("" if True else\n',
  ['test_peel_pipeline']),
 ('a receive-mode run waives its last store', 'GhostSpiral',
  '        waive=("" if receive_mode else\n',
  '        waive=("" if False else\n',
  ['test_peel_pipeline']),
 ('a waived store is not asked again when the run ends', 'GhostSpiral',
  '        atexit.register(_store_at_exit, rpc_primary)\n',
  '',
  ['test_peel_pipeline']),
 ('the exit ask says it stored whether or not it did', 'GhostSpiral',
  '        store_wallet(rpc_primary)\n        _ok = True\n    except Exception:                                        # noqa: BLE001\n        _ok = False\n',
  '        store_wallet(rpc_primary)\n        _ok = True\n    except Exception:                                        # noqa: BLE001\n        _ok = True\n',
  ['test_peel_pipeline']),
 ('store_wallet does not flush what it stored', 'gs_common.py',
  '        os.sync()\n        return\n',
  '        return\n',
  ['test_units', 'test_swap_receive']),
 ('store_wallet asks once', 'gs_common.py',
  '    _n = max(1, int(tries))\n',
  '    _n = 1\n',
  ['test_units']),
 ('store_wallet flushes before the store has answered', 'gs_common.py',
  '            rpc.raw_request("store", {})\n        except',
  '            os.sync()\n            rpc.raw_request("store", {})\n        except',
  ['test_units']),
 ('the receive is never stored', 'create_receive_wallet',
  '        store_wallet(wallet_rpc)\n',
  '        pass\n',
  ['test_swap_receive']),
 ('the receive is stored without the flush (the old raw call)', 'create_receive_wallet',
  '        store_wallet(wallet_rpc)\n',
  '        wallet_rpc.raw_request("store", {})\n',
  ['test_swap_receive']),
 ('a failed receive store stops the receive', 'create_receive_wallet',
  '        integrity_log("wallet", "store_failed")\n',
  '        integrity_log("wallet", "store_failed")\n        raise\n',
  ['test_swap_receive']),
 ('a failed receive store is not said on the terminal', 'create_receive_wallet',
  '        integrity_log("wallet", "store_failed")\n        print(f"  [!] The wallet did not write',
  '        integrity_log("wallet", "store_failed")\n        (lambda *a, **k: None)(f"  [!] The wallet did not write',
  ['test_swap_receive']),
 ('the fee sweep does not ask which wallet the fee wallet-rpc is serving', 'gs_wake_agent',
  '        _mis = fee_wallet_mismatch(key, cfg, connect=d.get("wallet_rpc"))\n        if _mis:\n',
  '        _mis = fee_wallet_mismatch(key, cfg, connect=d.get("wallet_rpc"))\n        if False:\n',
  ['test_wake_agent']),
 ('the fee sweep asks which wallet once, not before every leg', 'gs_wake_agent',
  '        if _mis:\n            integrity_log("wake", f"fee_sweep:{_mis}")\n',
  '        if _mis and legs == 0:\n            integrity_log("wake", f"fee_sweep:{_mis}")\n',
  ['test_wake_agent']),
 ('a refusal after a leg says nothing was spent', 'gs_wake_agent',
  '                      + ("Nothing was read or spent. " if legs == 0 else\n',
  '                      + ("Nothing was read or spent. " if True else\n',
  ['test_wake_agent']),
 ("a refusal after a leg leaves the leg's entry bundle behind", 'gs_wake_agent',
  '            if legs:\n                _retire_fee_bundle(artifact_dir)\n            return _mis\n',
  '            return _mis\n',
  ['test_wake_agent']),
 ("the fee wallet-rpc's endpoint is not compared with the mixing one", 'gs_wake_agent',
  '    if _endpoint(cfg["rpc"]) is not None \\\n            and _endpoint(cfg["rpc"]) == _endpoint(key.get("rpc_primary")):\n',
  '    if False:\n',
  ['test_wake_agent']),
 ('the fee wallet file is not compared with the mixing one', 'gs_wake_agent',
  '    if _signing_files(key.get("wallet_file")) \\\n            & _signing_files(cfg["wallet_file"]):\n        return "wallet_is_mixing"\n',
  '',
  ['test_wake_agent']),
 ('a wallet-rpc that names no primary address has answered', 'gs_wake_agent',
  '    if _p_mix is None or _p_fee is None:\n        return "wallet_unverified"\n',
  '',
  ['test_wake_agent']),
 ('the primary addresses are asked and not compared', 'gs_wake_agent',
  '    if _p_mix == _p_fee:\n        return "wallet_is_mixing"\n',
  '',
  ['test_wake_agent']),
 ('the mixing wallet owning the fee address is not refused', 'gs_wake_agent',
  '    if _mix is True:\n        return "wallet_is_mixing"\n',
  '',
  ['test_wake_agent']),
 ("the mixing wallet-rpc's silence is read as foreign (the review's F1)", 'gs_wake_agent',
  '    if _mix is None:\n        return "wallet_unverified"\n',
  '',
  ['test_wake_agent']),
 ('the fee wallet-rpc need not answer for the fee address', 'gs_wake_agent',
  '    if _wallet_owns(key, cfg["rpc"], cfg["address"], connect) is not True:\n        return "wallet_unverified"\n',
  '',
  ['test_wake_agent']),
 ('the fee-address probe lets connect_rpc exit the agent', 'gs_wake_agent',
  '                               proxy_url=key.get("tor_proxy", ""),\n                               connect=connect)\n    except SystemExit:\n        return None\n',
  '                               proxy_url=key.get("tor_proxy", ""),\n                               connect=connect)\n    except KeyboardInterrupt:\n        return None\n',
  ['test_wake_agent']),
 ('the primary-address probe lets connect_rpc exit the agent', 'gs_wake_agent',
  '                                  connect=connect)\n    except SystemExit:\n        return None\n',
  '                                  connect=connect)\n    except KeyboardInterrupt:\n        return None\n',
  ['test_wake_agent']),
 ("wallet-rpc's -2 is not read as foreign", 'gs_common.py',
  '        return False if _Foreign is not None and isinstance(e, _Foreign) \\\n            else None\n',
  '        return None\n',
  ['test_wake_agent']),
 ('every wallet-rpc error is read as foreign (the old reading)', 'gs_common.py',
  '        return False if _Foreign is not None and isinstance(e, _Foreign) \\\n            else None\n',
  '        return False\n',
  ['test_wake_agent']),
 ('a bool index is an answer', 'gs_common.py',
  '                    and not isinstance(ix.get(k), bool)\n',
  '',
  ['test_wake_agent']),
 ('an empty primary address is an answer', 'gs_common.py',
  '    a = a.strip() if isinstance(a, str) else ""\n    return a or None\n',
  '    a = a.strip() if isinstance(a, str) else ""\n    return a\n',
  ['test_wake_agent']),
 ('a trailing slash reads a keys file as a directory (os.path.split)', 'gs_common.py',
  '    pp = Path(p).expanduser()\n    if pp.name.endswith(".keys") and pp.name != ".keys":\n',
  '    pp = Path(os.path.expanduser(p))\n    if not p.endswith("/") and pp.name.endswith(".keys") and pp.name != ".keys":\n',
  ['test_wake_agent']),
 ('a wallet named with .keys is not the wallet named without', 'gs_common.py',
  '    if pp.name.endswith(".keys") and pp.name != ".keys":\n        pp = pp.with_name(pp.name[:-len(".keys")])\n',
  '',
  ['test_wake_agent']),
 ('a linked keys file is not followed', 'gs_common.py',
  '    return {k[:-len(".keys")], os.path.realpath(k)}\n',
  '    return {k[:-len(".keys")], k}\n',
  ['test_wake_agent']),
 ('a wallet-rpc URL with no port has no port', 'gs_common.py',
  '        return (_h, _p.port or 18083) if _h else None\n',
  '        return (_h, _p.port) if _h else None\n',
  ['test_wake_agent']),
 ('localhost and 127.0.0.1 are two hosts', 'gs_common.py',
  '        if _h in ("localhost", "::1") or _h.startswith("127."):\n            _h = "loopback"\n        return (_h, _p.port or 18083)',
  '        return (_h, _p.port or 18083)',
  ['test_wake_agent']),
 ('pairing reads a silent mixing wallet-rpc as a different wallet', 'gs_wake_keys',
  '    if _p_mix is None or _p_fee is None:\n        sys.exit(f"[!] the wallet-rpc at "\n',
  '    if False:\n        sys.exit(f"[!] the wallet-rpc at "\n',
  ['test_wake_agent']),
 ('pairing does not compare the primary addresses', 'gs_wake_keys',
  '    if _p_mix == _p_fee:\n        sys.exit("[!] the wallet-rpc at --fee-rpc is serving the SAME wallet "\n',
  '    if False:\n        sys.exit("[!] the wallet-rpc at --fee-rpc is serving the SAME wallet "\n',
  ['test_wake_agent']),
 ("pairing reads the mixing wallet-rpc's error on the fee address as foreign", 'gs_wake_keys',
  '    if _mine is None:\n        sys.exit("[!] the wallet-rpc at --rpc did not answer whether the fee "\n',
  '    if False:\n        sys.exit("[!] the wallet-rpc at --rpc did not answer whether the fee "\n',
  ['test_wake_agent']),
 ('pairing does not refuse a fee address the mixing wallet owns', 'gs_wake_keys',
  '    if _mine is True:\n        sys.exit("[!] the fee address minted from --fee-rpc belongs to the "\n',
  '    if False:\n        sys.exit("[!] the fee address minted from --fee-rpc belongs to the "\n',
  ['test_wake_agent']),
 ("pairing reads a wallet-rpc's error on a sweep destination as foreign", 'gs_wake_keys',
  '            if _o is None:\n                sys.exit(f"[!] the wallet-rpc of {_label} did not answer "\n',
  '            if False:\n                sys.exit(f"[!] the wallet-rpc of {_label} did not answer "\n',
  ['test_wake_agent']),
 ('pairing takes a wallet cache or a directory for the wallet', 'gs_wake_keys',
  '    if args.wallet_file and not os.path.isfile(\n            signing_keys_file(args.wallet_file)):\n',
  '    if args.wallet_file and not (os.path.exists(args.wallet_file)\n                                 or os.path.exists(args.wallet_file + ".keys")):\n',
  ['test_wake_agent']),
 ('pairing takes a fee wallet cache with no keys for the fee wallet', 'gs_wake_keys',
  '        if not os.path.isfile(signing_keys_file(args.fee_wallet_file)):\n',
  '        if not os.path.exists(args.fee_wallet_file):\n',
  ['test_wake_agent']),
 ('pairing compares the fee wallet-rpc URL as a string', 'gs_wake_keys',
  '        if rpc_endpoint(args.fee_rpc) is not None \\\n                and rpc_endpoint(args.fee_rpc) == rpc_endpoint(args.rpc):\n',
  '        if args.fee_rpc.rstrip("/") == str(args.rpc).rstrip("/"):\n',
  ['test_wake_agent']),
 ('pairing compares the wallet files as typed', 'gs_wake_keys',
  '        if signing_files(args.fee_wallet_file) \\\n                & signing_files(args.wallet_file):\n',
  '        if args.wallet_file and os.path.realpath(args.fee_wallet_file) \\\n                == os.path.realpath(args.wallet_file):\n',
  ['test_wake_agent']),
 ('a fee-wallet pairing is told it takes no fee', 'gs_wake_keys',
  '    if args.allow_withdraw and not (args.usage_fee_address or []) \\\n            and not args.fee_rpc:\n',
  '    if args.allow_withdraw and not (args.usage_fee_address or []):\n',
  ['test_wake_agent']),
 ("the old deposit's place is not released when the gate has room (15a68c4, reverted)", 'gs_wake_agent',
  '    if _btc_release is not None:\n        if _release_undelivered(artifact_dir, _btc_release,\n                                params.get("owner"), key=key,\n                                unused=d.get("btc_unused"))',
  '    if _btc_release is not None and _btc_release_needed:\n        if _release_undelivered(artifact_dir, _btc_release,\n                                params.get("owner"), key=key,\n                                unused=d.get("btc_unused"))',
  ['test_multi_client']),
 ('a used old address refuses a deposit the gate had room for', 'gs_wake_agent',
  '                                unused=d.get("btc_unused")) is not True \\\n                and _btc_release_needed:\n',
  '                                unused=d.get("btc_unused")) is not True:\n',
  ['test_multi_client']),
 ("a hand move's inputs the window did not name are not asked about", 'btc_forwarder',
  '            _ask = sorted(_unnamed - _kept_ops - _signed_ops)\n',
  '            _ask = []\n',
  ['test_btc_forwarder']),
 ("an input of this address the window missed is read as another address's", 'btc_forwarder',
  '                            _off[_o] = _sv\n',
  '                            _else_new.add(_o)\n',
  ['test_btc_forwarder']),
 ("an input nobody answered for is read as another address's", 'btc_forwarder',
  '                        else:\n                            _unknown.add(_o)\n',
  '                        else:\n                            _else_new.add(_o)\n',
  ['test_btc_forwarder']),
 ("an undecided move is the operator's hand", 'btc_forwarder',
  '            elif _failed:\n                _n_und = _und_counts.get(',
  '            elif False:\n                _n_und = _und_counts.get(',
  ['test_btc_forwarder']),
 ('an undecided move stops the look at the spends after it (the first version)', 'btc_forwarder',
  '                    _undecided.append((str(s["txid"]).lower(), _n_und))\n                    continue\n',
  '                    _undecided.append((str(s["txid"]).lower(), _n_und))\n                    break\n',
  ['test_btc_forwarder']),
 ('the moves set aside as undecided are never refused', 'btc_forwarder',
  '    if _undecided:\n        # COUNTED ON THE PLAN, so the next run knows how long this has been.\n',
  '    if False:\n        # COUNTED ON THE PLAN, so the next run knows how long this has been.\n',
  ['test_btc_forwarder']),
 ("an input of this address's own transactions that was not read is not over the line", 'btc_forwarder',
  '            _over = bool(_extra_ins & _signed_ops or _signed_hit or _unknown\n',
  '            _over = bool(_extra_ins & _signed_ops or _signed_hit\n',
  ['test_btc_forwarder']),
 ('an unnamed outpoint a plan of ours signed is not over the line', 'btc_forwarder',
  '            _over = bool(_extra_ins & _signed_ops or _signed_hit or _unknown\n',
  '            _over = bool(_extra_ins & _signed_ops or _unknown\n',
  ['test_btc_forwarder']),
 ('an unnamed outpoint a plan of ours signed is asked about', 'btc_forwarder',
  '            _ask = sorted(_unnamed - _kept_ops - _signed_ops)\n',
  '            _ask = sorted(_unnamed - _kept_ops)\n',
  ['test_btc_forwarder']),
 ('a move whose kept input the history did not name reads as a leaked seed', 'btc_forwarder',
  '        _hand = bool(_kept_ops and _all is not None and _all & _kept_ops)\n',
  '        _hand = bool(_kept_ops and _all is not None and _ins & _kept_ops)\n',
  ['test_btc_forwarder']),
 ('a txid that is not hex is put into the THORNode URL', 'btc_forwarder',
  '    if not _TXID_RE.match(str(txid or "").lower()):\n        return None\n    _t = str(txid).lower()\n',
  '    _t = str(txid).lower()\n',
  ['test_btc_forwarder']),
 ('a move whose bytes name nothing is read as the hand', 'btc_forwarder',
  '        _all = _spend_outpoints(s)\n        _hand = bool(',
  '        _all = _spend_outpoints(s) or set(_ins)\n        _hand = bool(',
  ['test_btc_forwarder']),
 ("an input recorded as another address's is asked about again", 'btc_forwarder',
  '            _unnamed = _all - _ins - _elsewhere\n',
  '            _unnamed = _all - _ins\n',
  ['test_btc_forwarder']),
 ('inputs of other addresses are not recorded', 'btc_forwarder',
  '                plan["hand_inputs_elsewhere"] = [list(o) for o in _pair_list(\n',
  '                plan["hand_inputs_elsewhere_x"] = [list(o) for o in _pair_list(\n',
  ['test_btc_forwarder']),
 ('what the window missed is not recorded as moved', 'btc_forwarder',
  '                for _o, _v in sorted(_off.items())]\n',
  '                for _o, _v in sorted(_off.items()) if False]\n',
  ['test_btc_forwarder']),
 ('what the window missed is not counted as consumed', 'btc_forwarder',
  '            _moved_ins |= _ins_all\n',
  '            _moved_ins |= _ins\n',
  ['test_btc_forwarder']),
 ("a spend's own bytes are trusted whatever txid they are listed under", 'btc_forwarder',
  '    if _tx.txid().hex() != str(s.get("txid") or "").lower():\n        return None\n    return {(i.txid.hex(), int(i.vout)) for i in _tx.vin}\n',
  '    return {(i.txid.hex(), int(i.vout)) for i in _tx.vin}\n',
  ['test_btc_forwarder']),
 ("input_sources reads a server's missing transaction as unknown (the first version)", 'gs_btc_broadcast.py',
  '                for t in fetch:\n                    got[t] = Transaction.parse(bytes.fromhex(b.transaction(t)))\n',
  '                for t in fetch:\n                    try:\n                        got[t] = Transaction.parse(bytes.fromhex(b.transaction(t)))\n                    except ServerError:\n                        got[t] = None\n',
  ['test_btc_broadcast']),
 ('input_sources fetches what the history does not list', 'gs_btc_broadcast.py',
  '                    if t in listed and t not in fetch and len(fetch) < _cap:\n',
  '                    if t not in fetch and len(fetch) < _cap:\n',
  ['test_btc_broadcast']),
 ('input_sources reads an input the history does not list as unknown', 'gs_btc_broadcast.py',
  '            if t not in listed:\n                out[(t, v)] = "unlisted"\n                continue\n',
  '            if t not in listed:\n                out[(t, v)] = None\n                continue\n',
  ['test_btc_broadcast']),
 ('input_sources fetches past its limit', 'gs_btc_broadcast.py',
  '                    if t in listed and t not in fetch and len(fetch) < _cap:\n',
  '                    if t in listed and t not in fetch:\n',
  ['test_btc_broadcast']),
 ("input_sources counts another address's output as this one's", 'gs_btc_broadcast.py',
  '            out[(t, v)] = (int(o.value) if o.script_pubkey.data == spk\n                           else False)\n',
  '            out[(t, v)] = int(o.value)\n',
  ['test_btc_broadcast', 'test_btc_forwarder']),
 ('input_sources asks about what the caller already knows', 'gs_btc_broadcast.py',
  '        if o not in _skip and o not in ask:\n',
  '        if o not in ask:\n',
  ['test_btc_broadcast']),
 ('the THORNode is never asked to settle an acceptor-only listing', 'btc_forwarder',
  '    if args.thornode and not shutdown_requested():\n        _th = thornode_inbound_height(',
  '    if False:\n        _th = thornode_inbound_height(',
  ['test_btc_forwarder']),
 ('an inbound ThorChain has not finalised is taken as a witness', 'btc_forwarder',
  '    if not (isinstance(fin, dict) and fin.get("completed") is True):\n        return None\n',
  '',
  ['test_btc_forwarder']),
 ("the THORNode's answer about another transaction is taken", 'btc_forwarder',
  '            or str(tx.get("id") or "").lower() != _t \\\n',
  '            or False \\\n',
  ['test_btc_forwarder']),
 ("the THORNode's answer about another chain is taken", 'btc_forwarder',
  '            or tx.get("chain") != _THOR_BTC_CHAIN:\n        return None\n    stages = st.get("stages")\n',
  '            or False:\n        return None\n    stages = st.get("stages")\n',
  ['test_btc_forwarder']),
 ("the observed transaction's height is not asked for, and a made-up one taken", 'btc_forwarder',
  '    h = ob.get("external_observed_height")\n',
  '    h = 1\n',
  ['test_btc_forwarder']),
 ("ThorChain's observed transaction about another txid is taken", 'btc_forwarder',
  '            or str(otx.get("id") or "").lower() != _t \\\n',
  '            or False \\\n',
  ['test_btc_forwarder']),
 ("a THORNode whose observed transaction cannot be read is not said on the chain", 'btc_forwarder',
  '        integrity_log("forward", f"txobserved_fetch_fail:{type(e).__name__}")\n',
  '',
  ['test_btc_forwarder']),
 ('a move undecided run after run is never the alarm (the review of fc7d108)', 'btc_forwarder',
  '                if _n_und < HAND_UNDECIDED_MAX:\n',
  '                if True:\n',
  ['test_btc_forwarder']),
 ('the undecided runs are not counted on the plan', 'btc_forwarder',
  '        plan["hand_undecided"] = _und_w\n',
  '',
  ['test_btc_forwarder']),
 ('a move decided at last leaves its undecided count behind', 'btc_forwarder',
  '                _und_p.pop(str(s["txid"]).lower(), None)\n',
  '                pass\n',
  ['test_btc_forwarder']),
 ("one server's 'unlisted' is recorded for good (the review of fc7d108)", 'btc_forwarder',
  '                            continue\n                        if _sv is False:\n',
  '                            _else_new.add(_o)\n                            continue\n                        if _sv is False:\n',
  ['test_btc_forwarder']),
 ("input_sources gives the server's 'unlisted' as a checked False", 'gs_btc_broadcast.py',
  '                out[(t, v)] = "unlisted"\n',
  '                out[(t, v)] = False\n',
  ['test_btc_broadcast']),
 ('a receive-mode run is told about an entry and a swap it does not have', 'GhostSpiral',
  '                    "--receive-wallet." if receive_mode else\n',
  '                    "--receive-wallet." if False else\n',
  ['test_peel_pipeline']),
 ('the wallet is asked to store only after the mix accounts are minted', 'GhostSpiral',
  '        persist_before_mint(rpc_primary)\n        subs, addr_index, decoy_addrs = create_subs(\n            rpc_primary, args.wallets, _num_decoys)\n',
  '        subs, addr_index, decoy_addrs = create_subs(\n            rpc_primary, args.wallets, _num_decoys)\n        persist_before_mint(rpc_primary)\n',
  ['test_peel_pipeline']),
 ("the acceptor's height is recorded, not ThorChain's", 'btc_forwarder',
  '            hit["server"], hit["height"] = "thornode", _th\n',
  '            hit["server"] = "thornode"\n',
  ['test_btc_forwarder']),
 ("the tx status is asked on the inbound list's circuit", 'btc_forwarder',
  '    proxies = isolated_proxy(proxy_url, "forward:txstatus")\n',
  '    proxies = isolated_proxy(proxy_url, "forward:inbound")\n',
  ['test_btc_forwarder']),
 ('a THORNode that cannot be read is not said on the chain', 'btc_forwarder',
  '        integrity_log("forward", f"txstatus_fetch_fail:{type(e).__name__}")\n',
  '',
  ['test_btc_forwarder']),
]
