...
MUTATIONS = [
 ...
 ('the fee sweep does not ask which wallet the fee wallet-rpc is serving', 'gs_wake_agent',
  '    _mis = fee_wallet_mismatch(key, cfg, injected=d.get("address_index"))\n    if _mis:\n',
  '    _mis = fee_wallet_mismatch(key, cfg, injected=d.get("address_index"))\n    if False:\n',
  ['test_wake_agent']),
 ("the fee wallet-rpc's endpoint is not compared with the mixing one", 'gs_wake_agent',
  '    if _rpc_endpoint(cfg["rpc"]) is not None \\\n',
  '    if False and _rpc_endpoint(cfg["rpc"]) is not None \\\n',
  ['test_wake_agent']),
 ('localhost and 127.0.0.1 are two hosts', 'gs_wake_agent',
  '        if _h in ("localhost", "::1") or _h.startswith("127."):\n            _h = "loopback"\n',
  '',
  ['test_wake_agent']),
 ('the fee wallet file is not compared with the mixing one', 'gs_wake_agent',
  '    if _wf and os.path.realpath(_wf) == os.path.realpath(cfg["wallet_file"]):\n',
  '    if _wf and _wf == cfg["wallet_file"]:\n',
  ['test_wake_agent']),
 ('the mixing wallet is not asked whether it owns the fee address', 'gs_wake_agent',
  '    if _wallet_owns(key, key.get("rpc_primary"), cfg["address"], injected):\n        return "wallet_is_mixing"\n',
  '',
  ['test_wake_agent']),
 ('the fee wallet-rpc need not answer for the fee address', 'gs_wake_agent',
  '    if not _wallet_owns(key, cfg["rpc"], cfg["address"], injected):\n        return "wallet_unverified"\n',
  '',
  ['test_wake_agent']),
 ('a bool index is an answer', 'gs_wake_agent',
  '    return (isinstance(ix, dict) and isinstance(ix.get("major"), int)\n            and not isinstance(ix.get("major"), bool))\n',
  '    return (isinstance(ix, dict) and isinstance(ix.get("major"), int))\n',
  ['test_wake_agent']),
 ('the wallet probe lets connect_rpc exit the agent', 'gs_wake_agent',
  '    except (Exception, SystemExit):                          # noqa: BLE001\n        return False\n    return (isinstance(ix, dict)',
  '    except Exception:                                        # noqa: BLE001\n        return False\n    return (isinstance(ix, dict)',
  ['test_wake_agent']),
 ("the old deposit's BTC address is asked even when the gate has room", 'gs_wake_agent',
  '    if _btc_release is not None and _btc_release_needed:\n        if _release_undelivered(',
  '    if _btc_release is not None:\n        if _release_undelivered(',
  ['test_multi_client']),
 ('a release the gate did not need is not said on the chain', 'gs_wake_agent',
  '    elif _btc_release is not None:\n        integrity_log("wake", "release_btc_not_needed")\n',
  '',
  ['test_multi_client']),
