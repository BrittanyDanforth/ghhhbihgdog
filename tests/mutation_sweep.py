...
MUTATIONS = [
 ...
 ('pairing accepts the sending switch without the signing switch', 'gs_wake_keys',
  ...
  '    if False:',
  ['test_wake_agent']),
 ('the two forward words leave the closed vocabulary', 'gs_wake_proto.py', '          "more_locked", "moved", "partial", "full", "sent", "unsure",\n          "delayed", "returned", "forwarded", "kept", "leftover")', '          "more_locked", "moved", "partial", "full",\n          "delayed", "returned", "forwarded", "kept", "leftover")', ['test_wake_agent']),
 ...
 ('pairing takes a negative returns bound',
  ...
  ['test_wake_agent']),
 ("the wire does not know `kept` (the doorbell refuses the vault's report)", 'gs_wake_proto.py', '          "delayed", "returned", "forwarded", "kept", "leftover")', '          "delayed", "returned", "forwarded", "leftover")', ['test_wake_agent']),
 ...
 ('not_yet on a recovered entry leaves a looked-at entry with no address',
  ...
  '                if False:',
  ['test_telegram_pager']),
 ('the mined-forward word leaves the closed vocabulary', 'gs_wake_proto.py', '          "delayed", "returned", "forwarded", "kept", "leftover")', '          "delayed", "returned", "kept", "leftover")', ['test_wake_doorbell', 'test_wake_agent']),
 ...
 ('a forward that found nothing settled is reported as FAILED', 'gs_wake_agent',
  '            if _forward_status_word(artifact_dir, handle):\n                continue',
  '            if False:\n                continue',
  ['test_wake_agent']),
 ...
 ("one server's fee estimate is paid whatever ThorChain says", 'btc_forwarder',
  '            and not isinstance(_estimate, bool) and _estimate > _fcap:\n',
  '            and not isinstance(_estimate, bool) and False:\n',
  ['test_btc_forwarder']),
 ("the operator's own fee rate is capped too", 'btc_forwarder',
  '    _fcap = (thorchain_fee_cap(thor_entries, args.feerate_floor)\n             if not args.feerate_sat_vb else None)\n',
  '    _fcap = thorchain_fee_cap(thor_entries, args.feerate_floor)\n',
  ['test_btc_forwarder']),
