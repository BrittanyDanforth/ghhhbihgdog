...
MUTATIONS = [
 ...
 ("verify_signed trusts the signer's sighash cache", 'gs_btc_tx.py',
  ...
  '    ok = True',
  ['test_btc_tx']),
 ('the forwarder runs without --dry-run', 'btc_forwarder',
  '    if not args.dry_run:\n        raise Refused("not_dry_run",',
  '    if False:\n        raise Refused("not_dry_run",',
  ['test_btc_forwarder']),
 ...
 ('a finished forward re-ships the deposit slip to the phone', 'gs_wake_agent',
  ...
  '    if True:\n        slip = seal_slip_for_delivery(key, artifact_dir, status, handle)',
  ['test_wake_agent']),
 ('the agent composes the forward WITHOUT --dry-run', 'gs_wake_agent',
  '                "--dry-run",                       # stage 2: never broadcasts\n',
  '',
  ['test_wake_agent']),
 ...
 ('a client token may forward another client\'s deposit (the owner wall is gone)',
  ...
  '        if False:',
  ['test_wake_agent']),
 ('a handle already forwarded is forwarded again', 'gs_wake_agent',
 ...
 ('a paired BTC setting present and wrong is coerced, not refused', 'gs_wake_agent',
  ...
  '    if False:',
  ['test_wake_agent']),
 ('the plan file says broadcast: true', 'btc_forwarder',
  '        "broadcast": False,',
  '        "broadcast": True,',
  ['test_btc_forwarder']),
 ...
 ('the signed bytes are persisted in the plan file by default', 'btc_forwarder',
  '                   if (not signed or args.write_signed_hex) else None),',
  '                   if True else None),',
  ['test_btc_forwarder']),
