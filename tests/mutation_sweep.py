...
MUTATIONS = [
 ...
 ("a leftover is dropped for the moved plan's forwarded", 'gs_wake_agent',
  '            if _word == "leftover" and _forward_mined(',
  '            if False and _forward_mined(',
  ['test_wake_agent']),
 ('a leftover is said before the forward on record has mined', 'gs_wake_agent',
  '            if _word == "leftover" and _forward_mined(\n                    artifact_dir, handle, reader) and (\n                    outcome == "accepted"',
  '            if _word == "leftover" and (\n                    outcome == "accepted"',
  ['test_wake_agent']),
 ...
 ('a leftover answer holds nothing: the same refused forward every window', 'gs_telegram_pager',
  '                e["held_conf"] = _start_conf if _own else -1',
  '                e.pop("held_conf", None)',
  ['test_telegram_pager']),
 ...
 ('the bare-forwarded hold does not double', 'gs_telegram_pager',
  '                    self.btc_recheck_s) * float(2 ** min(_n, 4))',
